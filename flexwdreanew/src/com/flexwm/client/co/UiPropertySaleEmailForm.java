...
public class UiPropertySaleEmailForm extends UiForm {
	...
	public UiPropertySaleEmailForm(UiParams uiParams, int id) {
		super(uiParams, new BmoEmail(), id);
		this.bmoEmail = (BmoEmail)getBmObject();
	}
	
	public UiPropertySaleEmailForm(UiParams uiParams, int id, BmoEmail bmoEmail, BmoPropertySale bmoPropertySale) {
		super(uiParams, bmoEmail, id);
		this.bmoEmail = (BmoEmail)getBmObject();
		

		BmFilter emailByCustomer = new BmFilter();
		emailByCustomer.setValueFilter(bmoCustomerEmail.getKind(), bmoCustomerEmail.getCustomerId(), bmoPropertySale.getBmoCustomer().getId());
	...
	public void populateFields() {
		...
		formFlexTable.addField(2, 2, toNameTextBox, bmoEmail.getToName());
		
		formFlexTable.addField(3, 0, cpTextBox, bmoEmail.getCp());
		formFlexTable.addField(3, 2, replyToTextBox, bmoEmail.getReplyTo());
		...
		formFlexTable.addFieldReadOnly(4, 0, fromTextBox, bmoEmail.getFrom());
		formFlexTable.addFieldReadOnly(4, 2, fromNameTextBox, bmoEmail.getFromName());
		
		formFlexTable.addField(5, 0, subjectTextBox, bmoEmail.getSubject());
		formFlexTable.addFieldReadOnly(5, 2, fixedBodyTextBox, bmoEmail.getFixedBody());
		
		formFlexTable.addField(6, 0, bodyTextArea, bmoEmail.getBody());
