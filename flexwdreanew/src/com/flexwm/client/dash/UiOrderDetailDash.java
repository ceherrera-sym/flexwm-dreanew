...
public class UiOrderDetailDash extends UiDashboard {
	...
	BmField amount = new BmField("totalExchange", "", "Total", 10, Types.DOUBLE, true, BmFieldType.CURRENCY, false);
	BmField total = new BmField("totalExchange", "", "Total", 10, Types.DOUBLE, true, BmFieldType.CURRENCY, false);
	BmField payments = new BmField("totalExchange", "", "Total", 10, Types.DOUBLE, true, BmFieldType.CURRENCY, false);
	BmField balance = new BmField("totalExchange", "", "Total", 10, Types.DOUBLE, true, BmFieldType.CURRENCY, false);

	private NumberFormat numberFormat = NumberFormat.getCurrencyFormat();

	public UiOrderDetailDash(UiParams uiParams) {
		super(uiParams, "Tablero Pedidos SCRUM", GwtUtil.getProperUrl(uiParams.getSFParams(), "/icons/scrm.png"));
	...
	private void setWest() {
		...
		addActionLabel("Inicio", "crmo", new ClickHandler() {
			public void onClick(ClickEvent event) {
				UiOrderDetailDash uiOrderSaleDash = new UiOrderDetailDash(getUiParams());
	...
	public void setDashboard(BmoUser bmoUser) {
		...
		filterUserLeader.setInFilter(bmoOrderDetail.getKind(), bmoUser.getIdFieldName(), bmoOrderDetail.getLeaderUserId().getName(), "1" , "1");
		leaderUserListBox.addFilter(filterUserLeader);

		addFilterListBox(filterPanel, leaderUserListBox, bmoOrderDetail.getLeaderUserId());
		...
		filterAssignedUser.setInFilter(bmoOrderDetail.getKind(), bmoUser.getIdFieldName(), bmoOrderDetail.getAssignedUserId().getName(), "1" , "1");
		assignedUserListBox.addFilter(filterAssignedUser);
		addFilterListBox(filterPanel, assignedUserListBox, bmoOrderDetail.getAssignedUserId());

		// Filtro de Departamento
		BmoArea bmoArea = new BmoArea();
		areaListBox = new UiListBox(getUiParams(), bmoArea);
		addFilterListBox(filterPanel, areaListBox, bmoOrderDetail.getAreaId());

		// Filtro de Moneda
		BmoCurrency bmoCurrency = new BmoCurrency();
		currencyListBox = new UiListBox(getUiParams(), bmoCurrency);
		addFilterListBox(filterPanel, currencyListBox, bmoOrderDetail.getBmoOrder().getCurrencyId(), "");

		addDateRangeFilterListBox(filterPanel, bmoOrderDetail.getStartDate(), "", "");
		...
		if (initialRpcAttempt < 5) {
			...
			AsyncCallback<ArrayList<BmObject>> callback = new AsyncCallback<ArrayList<BmObject>>() {
				...
				public void onSuccess(ArrayList<BmObject> result) {
					...
					getSumCCPendingWithoutLinked(initialTotalText, "" + BmoOrderDetail.STATUS_INITIAL);
			...
			try {
				if (!isLoading()) {
					...
					filterByStatus.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getStatus(), "" + BmoOrderDetail.STATUS_INITIAL);
					filterList.add(filterByStatus);

					// Filtrar por cliente
					if (customerSuggestBox.getSelectedId() > 0) {
						BmFilter filterByCustomer = new BmFilter();
						filterByCustomer.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getBmoOrder().getCustomerId(), customerSuggestBox.getSelectedId());
					...
					if (!leaderUserListBox.getSelectedId().equals("") && !leaderUserListBox.getSelectedId().equals("0")) {
						...
						filterByLeader.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getLeaderUserId(), leaderUserListBox.getSelectedId());
					...
					if (!assignedUserListBox.getSelectedId().equals("") && !assignedUserListBox.getSelectedId().equals("0")) {
						...
						filterByAssigned.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getAssignedUserId(), assignedUserListBox.getSelectedId());
					...
					if (!areaListBox.getSelectedId().equals("") && !areaListBox.getSelectedId().equals("0")) {
						...
						filterByArea.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getAreaId(), areaListBox.getSelectedId());
					...
					filterByOrderType.setInFilter(bmoOrderType.getKind(), 
							bmoOrderDetail.getBmoOrder().getOrderTypeId().getName(), 
					...
					BmFilter startDateRangeFilter = getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).searchFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getStartDate().getName());
					if (!startDateRangeFilter.getKind().equals(""))
						filterList.add(startDateRangeFilter);

					getUiParams().getBmObjectServiceAsync().list(bmoOrderDetail.getPmClass(), filterList, callback);
				}
		...
		if (doingRpcAttempt < 5) {
			...
			AsyncCallback<ArrayList<BmObject>> callback = new AsyncCallback<ArrayList<BmObject>>() {
				...
				public void onSuccess(ArrayList<BmObject> result) {
					...
					getSumCCPendingWithoutLinked(doingTotalText, "" + BmoOrderDetail.STATUS_DOING);
			...
			try {
				if (!isLoading()) {
					ArrayList<BmFilter> filterList = new ArrayList<BmFilter>();
					BmFilter filterByStatus = new BmFilter();
					filterByStatus.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getStatus(), "" + BmoOrderDetail.STATUS_DOING);
					filterList.add(filterByStatus);
		...
		if (doneRpcAttempt < 5) {
			...
			AsyncCallback<ArrayList<BmObject>> callback = new AsyncCallback<ArrayList<BmObject>>() {
				...
				public void onSuccess(ArrayList<BmObject> result) {
					...
					getSumCCPendingWithoutLinked(doneTotalText, "" + BmoOrderDetail.STATUS_DONE);
			...
			try {
				if (!isLoading()) {
					ArrayList<BmFilter> filterList = new ArrayList<BmFilter>();
					BmFilter filterByStatus = new BmFilter();
					filterByStatus.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getStatus(), "" + BmoOrderDetail.STATUS_DONE);
					filterList.add(filterByStatus);
		...
		if (holdRpcAttempt < 5) {
			...
			AsyncCallback<ArrayList<BmObject>> callback = new AsyncCallback<ArrayList<BmObject>>() {
				...
				public void onSuccess(ArrayList<BmObject> result) {
					...
					getSumCCPendingWithoutLinked(holdTotalText, "" + BmoOrderDetail.STATUS_HOLD);
				}
			};
			try {
				if (!isLoading()) {
					ArrayList<BmFilter> filterList = new ArrayList<BmFilter>();
					BmFilter filterByStatus = new BmFilter();
					filterByStatus.setValueFilter(bmoOrderDetail.getKind(), bmoOrderDetail.getStatus(), "" + BmoOrderDetail.STATUS_HOLD);
					filterList.add(filterByStatus);
	...
	private void setListData(Panel panel, Iterator<BmObject> orderDetailIterator, Label totalText) {
		//double total = 0; 
		int count = 0;

		while (orderDetailIterator.hasNext()) {
			BmoOrderDetail nextOrderDetail = (BmoOrderDetail)orderDetailIterator.next();

			Label codeLabel = new Label(nextOrderDetail.getBmoOrder().getCode().toString());
			codeLabel.setStyleName("listCellLink");
			codeLabel.addClickHandler(new UiClickHandler(nextOrderDetail) {
				@Override
				public void onClick(ClickEvent arg0) {
					BmoOrderDetail bmoOrderDetail = (BmoOrderDetail)bmObject;
					UiOrderDetail uiOrderDetail = new UiOrderDetail(getUiParams(), bmoOrderDetail.getOrderId().toInteger());
					uiOrderDetail.show();
				}
			});

			Label nameLabel = new Label(nextOrderDetail.getBmoOrder().getName().toString());

			Label customerLabel = new Label(nextOrderDetail.getBmoOrder().getBmoCustomer().getCode()
					+ " " + nextOrderDetail.getBmoOrder().getBmoCustomer().getDisplayName());
	...
	private void displayList(UiListFlexTable listFlexTable, Iterator<BmObject> iterator) {
		...
		BmoOrderDetail nextOrderDetail = new BmoOrderDetail();
		int row = 1;
		while (iterator.hasNext()) {
			col = 0;
			nextOrderDetail = (BmoOrderDetail)iterator.next();

			listFlexTable.addListCell(row, col++, getBmObject(), nextOrderDetail.getBmoOrder().getCode());
			listFlexTable.addListCell(row, col++, getBmObject(), nextOrderDetail.getBmoOrder().getName());
			listFlexTable.addListCell(row, col++, getBmObject(), nextOrderDetail.getBmoOrder().getBmoCustomer().getDisplayName());
			listFlexTable.addListCell(row, col++, getBmObject(), nextOrderDetail.getBmoOrder().getLockStart());
			listFlexTable.addListCell(row, col++, getBmObject(), nextOrderDetail.getBmoOrder().getBmoCurrency().getCode());

			// Hacer conversión de acuerdo a la moneda seleccionada
			if (Integer.parseInt(currencyListBox.getSelectedId()) == nextOrderDetail.getBmoOrder().getCurrencyId().toInteger()) {
				amountExchange = nextOrderDetail.getBmoOrder().getAmount().toDouble();
				totalExchange = nextOrderDetail.getBmoOrder().getTotal().toDouble();
				paymentsExchange = nextOrderDetail.getBmoOrder().getPayments().toDouble();
				balanceExchange = nextOrderDetail.getBmoOrder().getBalance().toDouble();
			} else {
				amountExchange = (nextOrderDetail.getBmoOrder().getAmount().toDouble() * nextOrderDetail.getBmoOrder().getCurrencyParity().toDouble()
						/ ((BmoCurrency)currencyListBox.getSelectedBmObject()).getParity().toDouble());
				totalExchange = (nextOrderDetail.getBmoOrder().getTotal().toDouble() * nextOrderDetail.getBmoOrder().getCurrencyParity().toDouble()
						/ ((BmoCurrency)currencyListBox.getSelectedBmObject()).getParity().toDouble());
				paymentsExchange = (nextOrderDetail.getBmoOrder().getPayments().toDouble() * nextOrderDetail.getBmoOrder().getCurrencyParity().toDouble()
						/ ((BmoCurrency)currencyListBox.getSelectedBmObject()).getParity().toDouble());
				balanceExchange = (nextOrderDetail.getBmoOrder().getBalance().toDouble() * nextOrderDetail.getBmoOrder().getCurrencyParity().toDouble()
						/ ((BmoCurrency)currencyListBox.getSelectedBmObject()).getParity().toDouble());
			}
			try {
		...
		if (sumCCRpcAttempt < 5) {
			...
			AsyncCallback<BmUpdateResult> callback = new AsyncCallback<BmUpdateResult>() {
				...
				public void onSuccess(BmUpdateResult result) {
					...
					if (getStatusRpcAttempt().equals("" + BmoOrderDetail.STATUS_INITIAL)) {
						getDoingList(0);
						//initialTotalText.setText(result.getMsg());
					} if (getStatusRpcAttempt().equals("" + BmoOrderDetail.STATUS_DOING)) {
						//doingTotalText.setText(result.getMsg());
						getDoneList(0);
					} if (getStatusRpcAttempt().equals("" + BmoOrderDetail.STATUS_DONE)) {
						//doneTotalText.setText(result.getMsg());
						getHoldList(0);
					} if (getStatusRpcAttempt().equals("" + BmoOrderDetail.STATUS_HOLD)) {
	...
	public void addFilterListBox(Panel p, UiListBox uiListBox, BmField bmField, String defaultValue) {
		String filterValue = getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).checkFilter(bmoOrderDetail.getKind(), bmField.getName());
		...
		String label = getUiParams().getSFParams().getFieldListTitle(bmoOrderDetail.getProgramCode(), bmField);
		Label l = new Label(label);
		l.setStyleName("listFilterLabel");
	...
	protected ChangeHandler getFilterChangeHandler(UiListBox uiListBox, BmField bmField) {
		UiFilterChangeHandler c = new UiFilterChangeHandler(uiListBox, bmoOrderDetail, bmField) {
			public void onChange(ChangeEvent event) {
	...
	private void setListBoxFilter(UiListBox uiListBox, BmField bmField) {
		...
		if (index > 0 || !uiListBox.isNullableBmField()) {
			...
			if (selectedId > 0 || !uiListBox.isNullableBmField()) {
				...
				bmFilter.setValueLabelFilter(bmoOrderDetail.getKind(), 
						bmField.getName(), 
						bmoOrderDetail.getProgramLabel(),
						"=",
						selectedId,
						valueLabel);
				getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).addFilter(bmFilter);
			}  
		} else {
			...
			bmFilter.setValueLabelFilter(bmoOrderDetail.getKind(), 
					...
					0,
					"");
			getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).removeFilter(bmFilter);
		}
	}
	...
	public void addFilterSuggestBox(Panel p, UiSuggestBox uiSuggestBox, BmField bmField) {
		...
		BmFilter existingFilter = getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).searchFilter(bmoOrderDetail.getKind(), bmField.getName());
		...
		clearButton.addClickHandler(new UiDialogLookupClickHandler(uiLookup, uiSuggestBox, bmField) {
			public void onClick(ClickEvent event) {
				if (uiSuggestBox.isEnabled()) {
					...
					BmFilter existingFilter = getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).searchFilter(bmoOrderDetail.getKind(), bmField.getName());
					getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).removeFilter(existingFilter);
					reset();
				}
			}
		});
	...
	protected UiSuggestBoxAction getUiSuggestBoxAction(UiSuggestBox uiSuggestBox, BmField baseBmField) {
		UiSuggestBoxAction uiSuggestBoxAction = new UiSuggestBoxAction(uiSuggestBox, bmoOrderDetail, baseBmField) {
			@Override
	...
	public void addDateRangeFilterListBox(Panel p, BmField bmField, String defaultStartDate, String defaultEndDate) {
		...
		BmFilter dateRangeFilter = getUiParams().getUiProgramParams(bmoOrderDetail.getProgramCode()).searchFilter(bmoOrderDetail.getKind(), bmField.getName());
	...
	protected ValueChangeHandler<String> getDateRangeFilterTextChangeHandler(UiDateBox startDateBox, UiDateBox endDateBox, BmField bmField) {
		UiFilterDateRangeTextChangeHandler c = new UiFilterDateRangeTextChangeHandler(startDateBox, endDateBox, bmoOrderDetail, bmField) {
	...
	protected ValueChangeHandler<Date> getDateRangeFilterChangeHandler(UiDateBox startDateBox, UiDateBox endDateBox, BmField bmField) {
		UiFilterDateRangeChangeHandler c = new UiFilterDateRangeChangeHandler(startDateBox, endDateBox, bmoOrderDetail, bmField) {
	...
	private void setDateRangeFilter(BmField baseBmField, UiDateBox startDateBox, UiDateBox endDateBox) {
		...
		if (startDateBox.getTextBox().getText().length() > 0
			...
			BmFilter bmFilter = new BmFilter();
			bmFilter.setRangeFilter(bmoOrderDetail.getKind(),
					baseBmField.getName(), 
					baseBmField.getLabel(), 
			...
			BmFilter bmFilter = new BmFilter();
			bmFilter.setValueOperatorFilter(bmoOrderDetail.getKind(),
					baseBmField,
					BmFilter.MAJOREQUAL,
