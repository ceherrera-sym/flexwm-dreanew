...
public class UiConsultancyEmailForm extends UiForm {
	...
	public UiConsultancyEmailForm(UiParams uiParams, int id) {
		super(uiParams, new BmoEmail(), id);
		this.bmoEmail = (BmoEmail)getBmObject();
	}

	public UiConsultancyEmailForm(UiParams uiParams, int id, BmoEmail bmoEmail, BmoConsultancy bmoConsultancy) {
		super(uiParams, bmoEmail, id);
		this.bmoEmail = (BmoEmail)getBmObject();
		this.bmoConsultancy = bmoConsultancy;

		BmFilter emailsByCustomer = new BmFilter();
		emailsByCustomer.setValueFilter(bmoCustomerEmail.getKind(), bmoCustomerEmail.getCustomerId(), bmoConsultancy.getBmoCustomer().getId());
		customerEmailListBox = new UiListBox(getUiParams(), bmoCustomerEmail, emailsByCustomer);

		BmFilter contactsByCustomer = new BmFilter();
		contactsByCustomer.setValueFilter(bmoCustomerContact.getKind(), bmoCustomerContact.getCustomerId(), bmoConsultancy.getBmoCustomer().getId());
	...
	public void formListChange(ChangeEvent event) {
		if (event.getSource() == customerContactListBox) {
			if (customerContactListBox.getSelectedId().equals("") || customerContactListBox.getSelectedId().equals("0")) {
				toTextBox.setText(bmoConsultancy.getBmoCustomer().getEmail().toString());
				toNameTextBox.setText(bmoConsultancy.getBmoCustomer().getDisplayName().toString());
			} else {
