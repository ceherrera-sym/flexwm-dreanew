...
public class UiCustomerServicePropertyReport extends UiReport {
	...
	String orderSection = "Filtros de Pedidos";

	public UiCustomerServicePropertyReport(UiParams uiParams) {
		super(uiParams, new BmoCustomerService(), "/rpt/op/cuse_generalproperty_report.jsp", uiParams.getSFParams().getProgramTitle(new BmoCustomerService()));
		...
		development = new BmField("developmentid", "", "Desarrollo", 8, Types.INTEGER, false, BmFieldType.ID, false);		
	}

	@Override
	public void populateFields() {
		reportTypeListBox = new UiListBox(getUiParams());
		reportTypeListBox.addItem("Reporte General de Atención Clientes", "/rpt/op/cuse_generalproperty_report.jsp");
		reportTypeListBox.addItem("Reporte Agrupado por Cliente de A.T.", "/rpt/op/cuse_bycustomer_report.jsp");
		...
		formFlexTable.addField(10, 0, developmentSuggestBox, development);
		formFlexTable.addField(11, 0, statusListBox, bmoCustomerService.getStatus(), true);
		statusListBox.setSelectedIndex(-1);
		formFlexTable.addSectionLabel(12, 0, orderSection, 2);
		formFlexTable.addField(13, 0, orderSuggestBox, bmoCustomerService.getOrderId());
		formFlexTable.addField(14, 0, customerSuggestBox, bmoCustomerService.getBmoOrder().getCustomerId());
		formFlexTable.addField(15, 0, lockStartBox, bmoCustomerService.getBmoOrder().getLockStart());
		formFlexTable.addField(16, 0, lockEndBox, bmoCustomerService.getBmoOrder().getLockEnd());
