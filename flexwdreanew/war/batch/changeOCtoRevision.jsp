...
try {		
     ...
     sql = " SELECT * FROM requisitions " +
           ...
           " AND (reqi_datecreate < '2017-10-01' or reqi_datecreate is null)" + 
  		   " AND (reqi_authorizeduser = 0 OR reqi_authorizeduser is null) ";
    ...
    pmConn2.doFetch(sql);
     ...
     while (pmConn2.next()) {
    	...
    	//Revisar que no exista ROC
    	//if (!pmRequisition.existROC(pmConn, bmoRequisition, bmUpdateResult)) {
    		//Existen CxP
    		//if (!pmRequisition.existPacc(pmConn, bmoRequisition, bmUpdateResult)) {
    			//Existe Anticipo OC
//     			if (!pmRequisition.hasRequisitionAdvance(pmConn, bmoRequisition, bmUpdateResult)) {
//     				//Cambiar a en revision la OC
