...
<body class="default">
	...
	<br>
	<FORM action="flex_customerstatus.jsp" method="POST" name="listFilter">
		...
		<TR>
			<td>
				<%	
					...
					PmConn pmConnCustomerStatus = new PmConn(sFParams);
					...
					try {
						...
						PmCustomerStatus pmCustomerStatus = new PmCustomerStatus(sFParams);
						pmConn.open();
						pmConnCustomerStatus.open();
						pmConn.disableAutoCommit();
						sql = "select * from customercompanies ";
						...
						while (pmConn.next()) {
							...
							sql = "SELECT * FROM customerstatus " +
									" WHERE csta_companyid = "+ pmConn.getInt("cucp_companyid")+
									" AND csta_customerid = "+ pmConn.getInt("cucp_customerid");
							
// 							System.out.println("Empresa: "+  pmConn.getInt("cucp_companyid"));
// 							System.out.println("Cliente: "+  pmConn.getInt("cucp_customerid"));
							//System.out.println("sql2: "+ sql);
							pmConnCustomerStatus.doFetch(sql);
							//Si no encontro el registro insertar customerStatus
							if (!pmConnCustomerStatus.next()) {
								bmoCustomerStatus.getCustomerId().setValue(pmConn.getInt("cucp_customerid"));
								bmoCustomerStatus.getCompanyId().setValue(pmConn.getInt("cucp_companyid"));
								bmoCustomerStatus.getStatus().setValue(BmoCustomerStatus.STATUS_PROSPECT);
								pmCustomerStatus.saveSimple(bmoCustomerStatus, bmUpdateResult);
							%>
							<br> -------------------------------------<br>
							Cliente: <%=bmoCustomerStatus.getCustomerId()%> | <%=bmoCustomerStatus.getCompanyId()%>
							<%
