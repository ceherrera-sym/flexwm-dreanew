...
<%
	...
	String programTitle = "Ligar las CxC";
        ...
        <td colspan="4">  
            <TABLE cellpadding="0" cellspacing="0" border="1" rules=all width="90%">               
            	<tr valign="middle" align="center" class="">
                    ...
                    <td class="reportHeaderCell">Externa</td>
            ...
            <%           
        		while (inputData.hasMoreTokens() && i < 2000) {
        			...
        			//Recuperar valores        		
        			externaId = tabs.nextToken();
        			invoiceId = tabs.nextToken();       			    				
        			
    		%>
    			
    				<TR class="reportCellEven" width="100%">
    					<%=HtmlUtil.formatReportCell(sFParams, "" + i, BmFieldType.NUMBER)%>
    					<%=HtmlUtil.formatReportCell(sFParams, externaId, BmFieldType.STRING)%>
    					<%=HtmlUtil.formatReportCell(sFParams, invoiceId, BmFieldType.STRING)%>    					    				    
...
<% } else if (action.equals("populate")) {
	...
	try {		
		...
		while (inputData.hasMoreTokens() && i < 20000) { 
			...
			externalId = tabs.nextToken();
			invoiceId = tabs.nextToken();        			    				
			
			bmoRaccountLink = new BmoRaccountLink();
			bmoRaccountLink.getRaccountId().setValue(invoiceId.replace("CC-", "").trim());
			bmoRaccountLink.getForeignId().setValue(externalId.replace("CC-", "").trim());
	...
	response.sendRedirect("/batch/flex_revisionLinkedRacc.jsp?action=complete&errorsave=" + bmUpdateResult.errorsToString());
