...
<%
	...
	String programTitle = "Importacion de SIC";
...
%>
    ...
    <TR>
        <td colspan="4">  
            ...
            <%          
	            PmIndustry pmIndustry = new PmIndustry(sFParams);
				BmoIndustry bmoIndustry = new BmoIndustry();
        		...
        		while (inputData.hasMoreTokens() && i < 2000) {
        			...
        			if (!sic.trim().equals("empty")) {
	        			...
	        			sql = "SELECT * FROM industries WHERE indu_name = '" + sic.trim() + "'";
	        			pmConn.doFetch(sql);
	        			if (!pmConn.next()) {
	        				errors = "No existe el SIC: " + sic;
	        			}
        			}	
    		...
    		%>
    				...
    				<TR class="reportCellEven" width="100%">
    					...
    					<%=HtmlUtil.formatReportCell(sFParams, sic, BmFieldType.STRING)%>    					    				    
	...
	<TABLE cellpadding="0" cellspacing="0" border="0"  width="100%">
		<FORM action="flex_revisionSICinCust.jsp" method="POST" name="listFilter">	
...
<% } else if (action.equals("populate")) {
	...
	try {		
		...
		while (inputData.hasMoreTokens() && i < 20000) { 
			...
			if (!sic.equals("empty")) {
    			...
    			pmConn.doFetch(sql);
    			if (pmConn.next()) {
    				industryId = pmConn.getInt("indu_industryid");
    			}
    			
				if (industryId > 0) {
					sql = "UPDATE customers SET cust_industryid = " + industryId + 
						  " WHERE cust_referralcomments = " + codeCust + 
						  " AND cust_industryid is null";
					pmConn.doUpdate(sql);
				}
			}
		}
	...
	response.sendRedirect("/batch/flex_revisionSICinCust.jsp?action=complete&errorsave=" + bmUpdateResult.errorsToString());
