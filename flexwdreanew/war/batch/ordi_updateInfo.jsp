...
<%
	// Variables
	String programTitle = "Actualizar Información de Items de Pedidos(no actualiza sub-total del Grupo ni del Pedido)";
	    ...
	    <tr class="reportSubTitle">
	        <td colspan="1" align="left" width="99%" height="35" class="reportCellEven">
	          <li>Se recomienda elaborar preparaci&oacute;n en una hoja de calculo, seleccionar y pegar en el recuadro siguiente.</li>          
         		<li>Formato (| = celda): Pedido | ID ITEM | Cantidad | Clave Producto | Nombre Producto/Item | Tipo Producto | Precio | 
...
// 		pmConn2.open();
		...
		try {
			...
			String orde_code = "";
			String ordi_orderItemId = "";
			String ordi_quantity = "";
			String prod_code = "";
			String ordi_name = "";
			String prod_type = "";
			String ordi_price = "";
		        ...
		        <td colspan="4">  
		            ...
		            <%           
		        		while (inputData.hasMoreTokens() && i < 2000) {
		        			...
		        			// Recuperar valores        		
		        			orde_code = tabs.nextToken();
		        			ordi_orderItemId = tabs.nextToken();
		        			ordi_quantity = tabs.nextToken();
		        			prod_code = tabs.nextToken();
		        			ordi_name = tabs.nextToken();
		        			prod_type = tabs.nextToken();
		        			ordi_price = tabs.nextToken();
		        			...
		        			orde_code = orde_code.trim();
		        			ordi_orderItemId = ordi_orderItemId.trim();
		        			ordi_quantity = ordi_quantity.trim();
		        			prod_code = prod_code.trim();
		        			ordi_name = ordi_name.trim();
		        			prod_type = prod_type.trim();
		        			ordi_price = ordi_price.trim();
		        			...
		        			// Validar Pedido
		        			if (!orde_code.equals("empty")) {
			        			sql = " SELECT * FROM orders " +
			          			      " WHERE orde_code LIKE '" + orde_code + "'";
			        			pmConn.doFetch(sql);
			        			if (!pmConn.next()) {
			        				errors = "El Pedido no Existe: " + orde_code;
			        			}
		        			}
		        			
		        			// Validar ID item
		        			if (!ordi_orderItemId.equals("empty")) {
			        			sql = " SELECT * FROM orderitems " +
			          			      " WHERE ordi_orderitemid = " + ordi_orderItemId + "";
			        			pmConn.doFetch(sql);
			        			if (!pmConn.next()) {
			        				errors = "El ITEM no Existe: " + ordi_orderItemId;
			        			}
		        			}	
		    		...
		    		%>
		    				<TR class="reportCellEven" width="100%">
		    					...
		    					<%=HtmlUtil.formatReportCell(sFParams, orde_code, BmFieldType.STRING)%>
		    					<%=HtmlUtil.formatReportCell(sFParams, ordi_orderItemId, BmFieldType.STRING)%>
		    					<%=HtmlUtil.formatReportCell(sFParams, ordi_quantity, BmFieldType.STRING)%>
								<%=HtmlUtil.formatReportCell(sFParams, prod_code, BmFieldType.STRING)%>
								<%=HtmlUtil.formatReportCell(sFParams, ordi_name, BmFieldType.STRING)%>
								<%=HtmlUtil.formatReportCell(sFParams, prod_type, BmFieldType.STRING)%>
		    					<%=HtmlUtil.formatReportCell(sFParams, ordi_price, BmFieldType.STRING)%>
			...
			while (inputData.hasMoreTokens() && i < 20000) {
				...
				area_name = area_name.trim();	
       			...
       			if (!ordi_orderItemId.equals("empty")) {
        			...
        			pmConn2.doFetch(sql);
        			if (pmConn2.next()) 
        				bmoOrderItem = (BmoOrderItem)pmOrderItem.get(Integer.parseInt(ordi_orderItemId));
       			}
    			...
    			if (!area_name.equalsIgnoreCase("empty")) {
       			...
       			bmoOrderItem.getQuantity().setValue(ordi_quantity);
       			bmoOrderItem.getProductId().setValue(prod_productId);
       			bmoOrderItem.getName().setValue(ordi_name);
       			bmoOrderItem.getPrice().setValue(ordi_price);
       			bmoOrderItem.getAmount().setValue(
       					(bmoOrderItem.getPrice().toDouble() * bmoOrderItem.getQuantity().toDouble() * bmoOrderItem.getDays().toDouble() ));
       			bmoOrderItem.getBudgetItemId().setValue(bgit_budgetItemId);
       			bmoOrderItem.getAreaId().setValue(area_areaId);
