...
<%
	if(sFParams.hasRead(bmoProgram.getCode().toString())) {
		...
		int projId = 0;
		String sql = "SELECT COUNT(proj_projectid) as c FROM projects WHERE proj_projectid = " + orderId;

		pmConnProj.doFetch(sql);

		if (pmConnProj.next()) projId = pmConnProj.getInt("c");

		if (projId > 0) {
			bmoProject = (BmoProject)pmProject.get(orderId);
	...
	%>
		...
		<%
			...
			PmWFlowType pmWFlowTypePropertySale = new PmWFlowType(sFParams);
			...
			bmoPropertySaleWFlowType = (BmoWFlowType)pmWFlowTypePropertySale.get(bmoPropertySale.getWFlowTypeId().toInteger());
