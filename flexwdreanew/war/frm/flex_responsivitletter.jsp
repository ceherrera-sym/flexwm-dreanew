...
<%
try {
	String title = "Carta Responsiva de Seguridad Estructural";
	...
	<body class="default" <%= permissionPrint %> style="font-size: 12px">
		...
		<table width="80%" align="center"  style="font-size: 12px">
			...
			<tr>
				<td>
					<p align="justify">
					Por este conducto, emitimos la responsiva de seguridad estructural con una vigencia de dos a&ntilde;os
					en base a la legislaci&oacute;n enmarcada del C&oacute;digo Civil y de procedimientos
					del estado de
					<% if (bmoDevelopment.getCityId().toInteger() > 0) {%>
			        		<%= bmoCityDevelopment.getBmoState().getName().toString()%>,
			        <% } %>
					para la vivienda marcada con el n&uacute;mero
					<b><%= bmoProperty.getNumber().toString()%></b>
					de la calle <b><%= bmoProperty.getStreet().toString()%></b> 
					del fraccionamiento <b><%= bmoDevelopment.getName().toString()%></b>  
					en esta ciudad.
					</p>
					
					<p align="justify">
					La responsiva estructural, dejara de surtir efecto si el acreditado, le da cualquier tipo 
					de uso diferente a la vivienda, para la que fue construida, que es el de casa habitaci&oacute;n. 
					Tambi&eacute;n si el acreditado modifica, altera o incrementa en la vivienda terminada el proyecto
					estructural original de la misma.
		
					</p>
					
					<p align="left">
				        <% if (bmoDevelopment.getCityId().toInteger() > 0) {%>
				            <b><%= bmoCityDevelopment.getName().toString()%>, <%= bmoCityDevelopment.getBmoState().getName().toString()%>
				        <% }%>
					...
					<p align="center">
						<b>
						<%= bmoCompany.getLegalRep().toString()%><br>
		...
		<% 
				...
				String errorLabel = "Carta responsiva de seguridad estructural";
