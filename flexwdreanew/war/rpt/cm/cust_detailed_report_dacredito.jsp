...
<%
	...
	int programId = 0, customerId = 0, salesman = 0, referralId = 0, territoryId = 0, countOrderTypes = 0,
			industryId = 0, locationId = 0, birthdateByMonth = 0, maritalStatus = 0;
	...
	if (request.getParameter("cust_locationid") != null)
		locationId = Integer.parseInt(request.getParameter("cust_locationid"));
 ...
 %> &nbsp;
		...
		<%
			if (sFParams.isFieldEnabled(bmoCustomer.getTerritoryId())
							|| sFParams.isFieldEnabled(bmoCustomer.getLocationId())) {
		%>
		<tr class="reportCellEven">
			<%
				if (sFParams.isFieldEnabled(bmoCustomer.getTerritoryId())) {
			...
			%>
			<th class="reportCellText">Ubicaci&oacute;n</th>
			<%=HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams,
									"" + pmConn.getString("locations", "loct_name"), BmFieldType.STRING))%>
			<%
				} else {
			%>
			<td colspan="2">&nbsp;</td>
