...
<%
	...
	NumberFormat formatCurrency = NumberFormat.getCurrencyInstance(locale);
   	...
   	int programId = 0, customerId = 0, salesman = 0, referralId = 0, territoryId = 0, countOrderTypes = 0, birthdateByMonth = 0,
   			industryId = 0, locationId = 0,maritalStatus = 0;
    ...
    if (request.getParameter("cust_locationid") != null) locationId = Integer.parseInt(request.getParameter("cust_locationid"));
    ...
    if (locationId > 0) {
        where += " AND cust_locationid = " + locationId;
     //   filters += "<i>Regi&oacute;n: </i>" + request.getParameter("cust_locationidLabel") + ", ";
        filters += "<i>" + HtmlUtil.stringToHtml(
				sFParams.getFieldFormTitle(bmoProgram.getCode().toString(), bmoCustomer.getLocationId())) + ": </i>" +
				request.getParameter("cust_locationidLabel") + ", ";
    }
    ...
    sql = " SELECT COUNT(*) as contador FROM" + SQLUtil.formatKind(sFParams, " customers ") +
			...
			" LEFT JOIN " + SQLUtil.formatKind(sFParams, " locations")+" ON (loct_locationid = cust_locationid) " +
	...
	if(sFParams.hasPrint(bmoProgram.getCode().toString()) || !(exportExcel.equals("1"))){ %>
		...
		<table class="report" width="100%">
			...
			%>
		    	...
		    	<%	//} %>
		    	<%	//if (sFParams.isFieldEnabled(bmoCustomer.getLocationId())) { %>
	        ...
	        while(pmConnGroup.next()) {
				...
				<%	} %>
					...
					<%	//} %>
					<%	//if (sFParams.isFieldEnabled(bmoCustomer.getLocationId())) { %>
							<%//= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, "" + pmConnGroup.getString("loct_name"), BmFieldType.STRING)) %>
					<%	//} %>
					<%	if (sFParams.isFieldEnabled(bmoCustomer.getReferralId())) { %>
