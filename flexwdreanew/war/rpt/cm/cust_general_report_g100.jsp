...
<!-- 		    	<td class="reportHeaderCell" >Estatus</td> -->
		    	...
		    	<%	if (sFParams.isFieldEnabled(bmoWFlowLog.getComments())) { %>
		    			<td class="reportHeaderCell" ><%= HtmlUtil.stringToHtml(sFParams.getFieldFormTitle(bmoWFlowLog.getProgramCode(), bmoWFlowLog.getComments()))%> Bit&aacute;cora</td>
	    		<%	} %>
	    		
	    		<%	if (sFParams.isFieldEnabled(bmoWFlowLog.getLogdate())) { %>
		    			<td class="reportHeaderCell" ><%= HtmlUtil.stringToHtml(sFParams.getFieldFormTitle(bmoWFlowLog.getProgramCode(), bmoWFlowLog.getLogdate()))%> Bit&aacute;cora </td>
	    		<%	} %>
			</tr>
	        ...
	        while(pmConnGroup.next()) {
	        	...
	        	bmoUserParent = (BmoUser)pmUser.get(pmConnGroup.getInt("user_parentid"));
				%>
				<tr class="reportCellEven">
...
<%-- 					<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, "" + pmConnGroup.getString("titl_name"), BmFieldType.STRING)) %> --%>
					...
					<%	} %>
							<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams,"" + bmoUserParent.getCode().toString(),BmFieldType.CODE)) %>
<%-- 					<%	if (sFParams.isFieldEnabled(bmoCustomer.getBirthdate())) { %> --%>
...
<%-- 					<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, bmoCustomer.getStatus().getSelectedOption().getLabel(), BmFieldType.STRING)) %> --%>
					...
					<%
						// Sacar la ultima bitacora de flujo del cliente de tipo otros
						String sqlLog = "SELECT wflg_comments, wflg_logdate FROM wflowlogs LEFT JOIN wflows ON (wflw_wflowid = wflg_wflowid) "
										+ " WHERE wflg_type = '" + BmoWFlowLog.TYPE_OTHER + "'"
										+ " AND wflw_callerid = " + pmConnGroup.getInt("cust_customerid")
										+ " AND wflw_callercode = '" + bmoCustomer.getProgramCode() +  "'"
										+ " ORDER BY wflg_logdate DESC LIMIT 1"	;
					
						//  System.out.println("sqlLog:"+sqlLog);
						pmConnExtra.doFetch(sqlLog);
						String comments = "", lastLogDate = "";
						while (pmConnExtra.next()) {
							comments = pmConnExtra.getString("wflg_comments");
							lastLogDate = pmConnExtra.getString("wflg_logdate");
						}
					%>
					<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, comments, BmFieldType.STRING)) %>
					<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, lastLogDate, BmFieldType.STRING)) %>
		</tr>
	   			<%	
   				i++;
	        } // pmConnGroup
