...
<%
	// Inicializar variables
 	String title = "Oportunidades por Cliente";
	...
	// Filtro agrupacion
	if (customerId > 0) {
   		groupFilter += " AND oppo_customerid = " + customerId;
...
//   	}
    ...
    if (disclosureFilters.length() > 0)
   	...
   	sql = " select cust_code, count(oppo_opportunityid) as c, sum(oppo_amount) as s, sum(quot_total) as q " + 
   			 " FROM opportunities " +
			 " LEFT JOIN customers on (oppo_customerid = cust_customerid) " +
			 " LEFT JOIN referrals on (refe_referralid = cust_referralid) " +
			 " LEFT JOIN quotes ON (quot_quoteid = oppo_quoteid) " +
			 " LEFT JOIN quoteproperties ON (qupy_quoteid = quot_quoteid) " +
			 " LEFT JOIN properties ON (prty_propertyid = qupy_propertyid) " +
			 " LEFT JOIN propertymodels ON (ptym_propertymodelid = oppo_propertymodelid) " +
			 " LEFT JOIN developments ON (deve_developmentid = ptym_developmentid) " +
     		 " LEFT JOIN users u1 ON(u1.user_userid = oppo_userid) " +
     		 " LEFT JOIN users u2 ON(u2.user_userid = u1.user_parentid) " +
			 " LEFT JOIN users u3 ON(u3.user_userid= u2.user_parentid) " +
			 " LEFT JOIN users u4 ON(u4.user_userid= u3.user_parentid) " +
			 " LEFT JOIN users u5 ON(u5.user_userid = u4.user_parentid) " +
		     " LEFT JOIN wflows on (wflw_wflowid = oppo_wflowid) " +
		     " LEFT JOIN wflowtypes wflowtypes ON (wflowtypes.wfty_wflowtypeid = oppo_wflowtypeid) " +
			 ...
			 " LEFT JOIN wflowtypes credito ON (credito.wfty_wflowtypeid = oppo_foreignwflowtypeid) " +  
			 " where wfca_programid = " + programId + 
			 where + groupFilter + 
			 " group by cust_code" +
			 " order by c desc, cust_code";
	...
	sql = " SELECT COUNT(cust_customerid) AS contador " + 
			" FROM opportunities " + 
			...
			where + groupFilter + 
			" ORDER BY cust_code ";
	System.out.println("contador"+sql);
...
<head>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
       ...
       %>
       		<%= "['" + pmConnChart.getString("cust_code") + "', " + pmConnChart.getInt("c") + "]," %>          
       <% 	
       		}
       %>
        ]);
	 ...
	 <script type="text/javascript">
      ...
      function drawChart() {
		...
		<% 
       		while(pmConnChart.next()) {
       %>
       		<%//= "['" + pmConnChart.getString("cust_code") + "', " + pmConnChart.getInt("s") + "]," %>
       		<%= "['" + pmConnChart.getString("cust_code") + "', " + pmConnChart.getInt("q") + "]," %>
       <%	
       		}
       %>
        ]);
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			<b>Filtros:</b> <%= filters %>
			<br>
			<b>Agrupado Por:</b> Cliente, <b>Ordenado por:</b> Fecha
		</td>
...
<table class="report">
    ...
    <% 	
	sql = " SELECT cust_customerid, cust_code, cust_displayname " + 
			" FROM opportunities " + 
			" LEFT JOIN customers on (cust_customerid = oppo_customerid) " +					 
			" LEFT JOIN quotes ON (quot_quoteid = oppo_quoteid) " +
			" LEFT JOIN quoteproperties ON (qupy_quoteid = quot_quoteid) " +
	...
	pmConnGroup.doFetch(sql);
        ...
        while(pmConnGroup.next()) {
			int groupCustId = pmConnGroup.getInt("cust_customerid");	
	    	...
	    	sql = " SELECT *, " +
	    			...
	    			" FROM opportunities " +
	    			" LEFT JOIN customers on (cust_customerid = oppo_customerid) " +
	    			" LEFT JOIN referrals on (cust_referralid = refe_referralid) " +
	    			" LEFT JOIN quotes ON (quot_quoteid = oppo_quoteid) " +
					" LEFT JOIN quoteproperties ON (qupy_quoteid = quot_quoteid) " +
					" LEFT JOIN properties ON (prty_propertyid = qupy_propertyid) " +
					" LEFT JOIN propertymodels ON (ptym_propertymodelid = oppo_propertymodelid) " +
	    			...
	    			" LEFT JOIN wflowcategories on (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
					" LEFT JOIN wflowtypes credito ON (credito.wfty_wflowtypeid = oppo_foreignwflowtypeid) " +
// 					" LEFT JOIN  customerphones ON (cuph_customerid = cust_customerid) " +	 
	    			" WHERE oppo_customerid = " + groupCustId +
	    			where + 
	    ...
	    %>
			    <tr>
					<td colspan="23" class="reportGroupCell">
					    <%= pmConnGroup.getString("cust_code") %>
					    <%= HtmlUtil.stringToHtml(pmConnGroup.getString("cust_displayname")) %> 
					</td>
				</tr>
			    <tr >
	          ...
	          <tr class="reportCellEven">
	        	  ...
	        	  String bitacora = "";
	        	  if(profileId > 0){
	        		  String bitacoraGrupo = "SELECT wfsp_commentlog FROM wflowsteps " +
	        	  ...
	        	  %>
	        	  <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(bitacora), BmFieldType.STRING)) %>
