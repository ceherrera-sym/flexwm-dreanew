...
//    	where += " AND oppo_companyid = " + sFParams.getSelectedCompanyId();
	...
	if (sFParams.getSelectedCompanyId() > 0) {
   	...
   	sql = " SELECT wflf_name AS Funnel, count(oppo_opportunityid) AS Oportunidades "
			+ "	FROM" + SQLUtil.formatKind(sFParams, " opportunities ")
			+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, " wflows")+" ON (oppo_wflowid = wflw_wflowid) "
			+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, " wflowfunnels")+" ON (wflw_wflowfunnelid = wflf_wflowfunnelid) "
			+ " WHERE oppo_opportunityid > 0 AND oppo_status = '" + BmoOpportunity.STATUS_REVISION + "' "
			+ where  
			+ " GROUP BY wflf_name";
...
<head>
    ...
    function drawChart() {
      var data = google.visualization.arrayToDataTable([
        	['Funnel', 'Oportunidades', { role: 'annotation' } ],
        	<% 
        		int total = 0;
     			while (pmConn.next()) {
     				total += pmConn.getInt("Oportunidades");
     		%>
	        		['<%= pmConn.getString("Funnel") %>', <%= pmConn.getInt("Oportunidades") %>, '<%= pmConn.getInt("Oportunidades") %>'],
      	<%
     			}
      	%>
      	]);
      ...
      var options = {
        chart: {
          title: 'Oportunidades',
          subtitle: 'Por Funnel, total: <%= total %>',
        },
