...
<%
	// Inicializar variables
 	String title = "Oportunidades Perdidas por Cliente";
...
//   	}
	...
	*/
    ...
    if (disclosureFilters.length() > 0)
   	...
   	sql = " SELECT cust_customerid,cust_code, count(oppo_opportunityid) as c, sum(oppo_amount) as s, sum(quot_total) as q " + 
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
       ...
       %>
        ...
        var options = {
          title: 'Oportunidades Perdidas por Cliente',
          vAxis: {minValue: 0},
	 ...
	 <script type="text/javascript">
      ...
      function drawChart() {
       ...
       %>
        ...
        var options = {
          title: 'Valor Total Perdido por Cliente',
          vAxis: {minValue: 0},
          animation: {duration: 10000, easing: "inAndOut"},
          legend: { position: "none"},
...
<table class="report">
    ...
    sql = "SELECT count(contador) AS contador FROM(Select Count(*) AS contador FROM " + SQLUtil.formatKind(sFParams, "opportunities") +
			...
			groupFilter + where  +
			" GROUP BY cust_customerid " + 
			" ORDER BY cust_customerid ASC) AS alias";
			int count =0;
			//ejecuto el sql
	...
	sql = " SELECT cust_customerid, cust_code, cust_displayname " + 
	    ...
	    %>
		    	<tr>
					<td colspan="17" class="reportGroupCell">
					    <%= pmConnGroup.getString("cust_code") %>
					    <%= HtmlUtil.stringToHtml(HtmlUtil.stringToHtml(pmConnGroup.getString("cust_displayname"))) %> 
					</td>
				</tr>
			    <tr class="">
...
%> 
	...
	<%}
	System.out.println("\n  fin de reporte Fecha: " +  SFServerUtil.nowToString(sFParams, sFParams.getDateTimeFormat()) 
	+ " Reporte: "+title
	+ " Usuario: " + sFParams.getLoginInfo().getEmailAddress()); //Fin de validacion de 5 mil registros
