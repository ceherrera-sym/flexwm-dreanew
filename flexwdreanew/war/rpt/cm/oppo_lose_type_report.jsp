...
try {
	// Inicializar variables
 	String title = "Oportunidades Perdidas Agrupadas por Tipo de Flujo";
	...
	if (loseMotiveId > 0) {
		where += " AND lomt_losemotiveid = " + loseMotiveId;
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			...
			<b>Ordenado por:</b> Tipo de Flujo, Clave Oportunidad
		</td>
	...
	if (!(currencyId > 0)) {
		...
		while (pmCurrencyWhile.next()) { %>
			<table class="report" width="100%">
				...
				chartSql = " SELECT wfty_wflowtypeid, wfty_name, count(oppo_opportunityid) as c, sum(quot_total) as s  " +
						" FROM " + SQLUtil.formatKind(sFParams, "opportunities ") + 
						" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
						" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
						" LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
						...
						groupFilter + where + 
						" GROUP BY wfty_wflowtypeid ORDER BY wfty_name ASC";
				...
				<script type="text/javascript">
					...
					function drawChart() {
						var data = google.visualization.arrayToDataTable([
	                          ['Tipo de Flujo', 'Oportunidades Perdidas por Tipo Flujo'],
	                          <% 
				...
				<tr>
					<td width="50%"> <div id="donutchart<%= pmCurrencyWhile.getString("cure_code")%>" style="width: 80%; height: 400px;"></div></td>
	...
	<%
		...
		// Agrupa oportunidades por tipo de flujo de x moneda
          		...
          		if(pmConnGroup.getInt("wflowtypes", "wfty_wflowtypeid") != wflowtypeid) {
			    	...
			    	<tr class="reportGroupCell">
			 			<td colspan="<%= 18 + dynamicColspan%>" class="reportGroupCell">
			    	  		<%= HtmlUtil.stringToHtml(pmConnGroup.getString("wfty_name")) %>
						</td>
					</tr>
				    <tr class="">
...
<%	} // Fin pmCurrencyWhile
	...
	<%
   	...
   	%>
	   	...
	   	<script type="text/javascript">
		   	...
		   	function drawChart() {
		   		var data = google.visualization.arrayToDataTable([
                      ['Tipo de Flujo', 'Oportunidades Perdidas por Tipo de Flujo'],
                      <% 
                      String statusCaptionDonutChart = "";
                      // Contar  las oportunidades perdidas por motivo para la grafica DONUT
                      chartSql = " SELECT wfty_wflowtypeid, wfty_name, count(oppo_opportunityid) as c " +
                    		  " FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
                    		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
                    		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
                    		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	 	...
	 	<script type="text/javascript">
	      ...
	      function drawChart() {
	        var data = google.visualization.arrayToDataTable([
	          ...
	          String statusCaptionTypeChart = "";
	          chartSql = " SELECT wfty_wflowtypeid, wfty_name " +
	        		  " FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	          ...
	          while (pmConnChartGroup.next()) {
	        	  sql = " SELECT quot_quoteid, quot_total, quot_currencyid, quot_currencyparity " +
	        			  ...
	        			  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
	        			  " WHERE wfca_programid = " + programId + 
	        			  " AND oppo_wflowtypeid = " + pmConnChartGroup.getInt("wflowtypes", "wfty_wflowtypeid") + 
	        			  where;
    ...
    <%
	    ...
	    // Filtra oportunidades perdidas por tipo de flujo
	    String sqlGroup = "SELECT distinct(wfty_wflowtypeid), wfty_name FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "venues")+" ON (venu_venueid = oppo_venueid) " +
  			  	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "losemotives")+" ON (oppo_losemotiveid = lomt_losemotiveid) " +
