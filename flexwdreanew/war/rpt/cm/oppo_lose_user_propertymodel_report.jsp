...
<%
	// Inicializar variables
 	String title = "Oportunidades Perdidas por Vendedor";
...
//   	}
   	...
   	if (sFParams.restrictData(bmoOpportunity.getProgramCode()) || (userId > 0)) {
   		if (!(userId > 0)) {
   			...
   			groupFilter = " AND user_userid = " + userId;
   		}
   		filters += "<i>Vendedor: </i>" + request.getParameter("oppo_useridLabel") + ", ";
...
<head>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
       ...
       %>
       		<%= "['" + pmConnChart.getString("user_email") + "', " + pmConnChart.getInt("c") + "]," %>          
       <%
       		}
       %>
        ]);
        ...
        var options = {
          title: 'Oportunidades Perdidas por Vendedor',
          vAxis: {minValue: 0},
	 ...
	 <script type="text/javascript">
      ...
      function drawChart() {
		...
		<% 
       		while(pmConnChart.next()) {
       %>
       		<%= "['" + pmConnChart.getString("user_email") + "', " + pmConnChart.getInt("q") + "]," %>          
       <%
       		}
       %>
        ]);
        ...
        var options = {
          title: 'Valor Total Perdido por Vendedor',
          vAxis: {minValue: 0},
          animation: {duration: 10000, easing: "inAndOut"},
          legend: { position: "none"},
...
<table class="report">
	...
	sql = " SELECT u1.user_userid, u1.user_firstname, u1.user_fatherlastname, u1.user_motherlastname, u1.user_email " +
			" FROM " + SQLUtil.formatKind(sFParams, " opportunities ") +
			" LEFT JOIN"+ SQLUtil.formatKind(sFParams," losemotives")+" on (lomt_losemotiveid = oppo_losemotiveid) " +	      
			" LEFT JOIN"+ SQLUtil.formatKind(sFParams," customers")+" on (cust_customerid = oppo_customerid) " +
			" LEFT JOIN"+ SQLUtil.formatKind(sFParams," referrals")+" on (refe_referralid = cust_referralid) " +
        ...
        while(pmConnGroup.next()) {
			int groupUserId = pmConnGroup.getInt("u1", "user_userid");	
	    ...
	    %>
			    ...
			    <tr>
					<td colspan="25" class="reportGroupCell">
					    <%= pmConnGroup.getString("u1", "user_email") %> - 
