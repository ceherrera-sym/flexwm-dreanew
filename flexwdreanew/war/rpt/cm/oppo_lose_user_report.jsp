...
try {
	// Inicializar variables
 	String title = "Oportunidades Perdidas Agrupadas por Vendedor";
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			...
			<b>Ordenado por:</b> Vendedor, Clave Oportunidad
		</td>
...
//if que muestra el mensajede error
	...
	if (!(currencyId > 0)) {
		...
		while (pmCurrencyWhile.next()) { %>
			...
			chartSql = " SELECT user_userid, user_code, user_firstname, user_fatherlastname, user_motherlastname, user_email, " +
					" count(oppo_opportunityid) as c, sum(quot_total) as s  " +
					" FROM "+ SQLUtil.formatKind(sFParams,"opportunities") + 
					" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
					" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
					" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"referrals")+" ON (cust_referralid = refe_referralid) " +
					...
					groupFilter + where + 
					" GROUP BY user_userid ORDER BY user_code";
			...
			<script type="text/javascript">
				...
				function drawChart() {
					var data = google.visualization.arrayToDataTable([
                          ['Vendedor', 'Oportunidades Perdidas por Vendedor'],
		       <% 
	...
	<%
		...
		String sqlGroup = "SELECT user_userid, user_code, user_firstname, user_fatherlastname, user_motherlastname, user_email " +
				" FROM "+ SQLUtil.formatKind(sFParams,"opportunities") + 
				" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
				" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
				" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"losemotives")+" ON (oppo_losemotiveid = lomt_losemotiveid) " +
				...
				" AND oppo_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
				groupFilter + where +
				" GROUP BY user_userid ORDER BY user_code ASC";
        ...
        while(pmConnGroup.next()) {
	    	...
	    	sql = " SELECT * FROM "+ SQLUtil.formatKind(sFParams,"opportunities") +
	    			...
	    			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
	    			" WHERE " +
	    			" oppo_userid = " + pmConnGroup.getInt("users", "user_userid") +
		   			" AND oppo_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
	    			where +
	    			" ORDER BY oppo_code ASC";         
...
<%	} // Fin pmCurrencyWhile
	...
	<table class="report" width="100%">
	    ...
	    // Filtra oportunidades perdidas por vendedor
