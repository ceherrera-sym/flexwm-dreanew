...
<%
	// Inicializar variables
 	String title = "Oportunidades por Fase";
...
//   	}
  ...
  //abro conexion para inciar el conteo consulta general
    ...
    sql=(" SELECT COUNT(*) as contador " + 
   	...
   	sql = " SELECT wfph_name, count(oppo_opportunityid) as c, sum(oppo_amount) as s, sum(quot_total) as q FROM " + SQLUtil.formatKind(sFParams, " opportunities ") +
			 " LEFT JOIN"+ SQLUtil.formatKind(sFParams," customers ")+" on (cust_customerid = oppo_customerid) " +
			 ...
			 " WHERE wfca_programid = " + programId + 
			 where + groupFilter +
			 " GROUP BY wfph_name ORDER BY wfph_sequence";
...
<head>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
       ...
       %>
       		<%= "['" + pmConnChart.getString("wflowphases", "wfph_name") + "', " + pmConnChart.getInt("c") + "]," %>          
       <%
       		}
       %>
        ]);
	 ...
	 <script type="text/javascript">
      ...
      function drawChart() {
		...
		<% 
       		while(pmConnChart.next()) {
       %>
       		<%//= "['" + pmConnChart.getString("wflowphases", "wfph_name") + "', " + pmConnChart.getInt("s") + "]," %>
       		<%= "['" + pmConnChart.getString("wflowphases", "wfph_name") + "', " + pmConnChart.getDouble("q") + "]," %>          
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			<b>Filtros:</b> <%= filters %>
			<br>
			<b>Agrupado Por:</b> Fase de Flujo, <b>Ordenado por:</b> Fecha
		</td>
...
<table class="report">
	...
	pmConnGroup.doFetch(" SELECT * " + 
						...
						" WHERE wfca_programid = " + programId + 
						where  + groupFilter + 
						" GROUP BY wfph_wflowphaseid " +
						" ORDER by wfph_sequence");
	
		int  i = 0, y = 0;
        while(pmConnGroup.next()) {
			int groupPhaseId = pmConnGroup.getInt("wflowphases", "wfph_wflowphaseid");	
	    	...
	    	sql = " SELECT *, " +
		    			...
		    			" FROM " + SQLUtil.formatKind(sFParams, " opportunities ") +
		    			" LEFT JOIN"+ SQLUtil.formatKind(sFParams," customers")+" on (oppo_customerid = cust_customerid) " +
						...
						" LEFT JOIN"+ SQLUtil.formatKind(sFParams," wflowtypes")+" wflowtypes ON (wflowtypes.wfty_wflowtypeid = oppo_wflowtypeid) " +  
		    			" LEFT JOIN"+ SQLUtil.formatKind(sFParams," wflowphases")+" on (wfph_wflowphaseid = wflw_wflowphaseid) " +	  
						" LEFT JOIN"+ SQLUtil.formatKind(sFParams," wflowtype")+"s credito ON (credito.wfty_wflowtypeid = oppo_foreignwflowtypeid) " +
// 						" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customerphones")+" ON (cuph_customerid = cust_customerid) " +	 

						" WHERE wflw_wflowphaseid = " + groupPhaseId +
						where + 
	          ...
	          while(pmConnList.next()) {
        		  ...
        		  <tr>
        		  	<td colspan="23" class="reportGroupCell">
					    <%= pmConnGroup.getString("wflowphases", "wfph_sequence") %>. 
