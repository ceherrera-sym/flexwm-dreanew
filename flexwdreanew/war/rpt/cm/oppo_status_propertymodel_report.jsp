...
<%
	...
	BmoOpportunity bmoOpportunity = new BmoOpportunity();
 	...
 	String title = "Oportunidades Por Estatus";
    ...
    // Filtro agrupacion
    if (!status.equals("")) {
    	//groupFilter = " AND oppo_status like '" + status + "'";
...
//   	}
	...
	*/
	...
	String sqlStatus= " SELECT distinct(oppo_status) as stat, sum(quot_total) as q " +
							" FROM " + SQLUtil.formatKind(sFParams, " opportunities ") +
							" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customers")+" ON (cust_customerid = oppo_customerid) " +
							" LEFT JOIN " + SQLUtil.formatKind(sFParams, " referrals")+" ON (cust_referralid = refe_referralid) " +
	...
	PmConn pmConnStatus = new PmConn(sFParams);
	pmConnStatus.open();
	pmConnStatus.doFetch(sqlStatus);
      ...
      function drawChart() {
		...
		<% 
			String statusCaptionTypeChart = "";
       		while(pmConnStatus.next()) {
          		bmoOpportunity.getStatus().setValue(pmConnStatus.getString("opportunities", "stat"));
       %>
       		<%= "['" + bmoOpportunity.getStatus().getSelectedOption().getLabel() + "', " + pmConnStatus.getDouble("q") + "]," %>          
       <%
       		}
       %>
        ]);
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			<b>Filtros:</b> <%= filters %>
			<br>
			<b>Agrupado Por:</b> Estatus, <b>Ordenado por:</b> Fecha
		</td>
...
<%
	...
	sql=("SELECT COUNT(*) as contador FROM " + SQLUtil.formatKind(sFParams, " opportunities  ") + 
  		 ...
  		 " LEFT JOIN " + SQLUtil.formatKind(sFParams, " users")+" u2 ON(u2.user_userid = u1.user_parentid) " +
		     ...
		     " LEFT JOIN " + SQLUtil.formatKind(sFParams, " wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		     " WHERE wfca_programid = " + programId + 
			 where + groupFilter + " GROUP BY oppo_status ORDER BY oppo_status ASC");
	int count =0;
...
<table class="report" width="100%">
	...
	pmConnGroup.doFetch("SELECT * FROM " + SQLUtil.formatKind(sFParams, " opportunities  ") + 
						 " LEFT JOIN " + SQLUtil.formatKind(sFParams, " customers")+" ON (cust_customerid = oppo_customerid) " +
						 " LEFT JOIN " + SQLUtil.formatKind(sFParams, " referrals")+" ON (cust_referralid = refe_referralid) " +
						 " LEFT JOIN " + SQLUtil.formatKind(sFParams, " quotes")+" ON (quot_quoteid = oppo_quoteid) " +
						 " LEFT JOIN " + SQLUtil.formatKind(sFParams, " quoteproperties")+" ON (qupy_quoteid = quot_quoteid) " +
...
// 						" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customerphones")+" ON (cuph_customerid = cust_customerid) " +	 
			...
			pmConnList.doFetch(sql);
	          ...
	          double total = 0;
	          String oppoStatus= "", statusCaption = "";
	    ...
	    %>
	          ...
	          <tr class="reportCellEven">
                ...
                <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, customer, BmFieldType.STRING)) %>
	                ...
	                %>
	                <%=  HtmlUtil.formatReportCell(sFParams, numeros, BmFieldType.DATETIME) %>
