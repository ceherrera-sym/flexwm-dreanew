...
<%
	...
	String title = "Oportunidades Agrupadas por Estatus";
    ...
    // Filtro agrupacion
    if (!status.equals("")) {
    	groupFilter = SFServerUtil.parseFiltersToSql("oppo_status", status);
...
<html>
	...
	<table border="0" cellspacing="0" cellpading="0" width="100%">
		...
		<tr>
			<td class="reportSubTitle">
				<b>Filtros:</b> <%= filters %>
				<br>
				<b>Agrupado por:</b> <%= ((!(currencyId > 0)) ? "Moneda, Estatus" : "Estatus")%>
				<b>Ordenado por:</b> Estatus, Clave Oportunidad
			</td>
	...
	if (!(currencyId > 0)) {
		...
		sql = " SELECT distinct(oppo_status) as stat, count(oppo_opportunityid) as c, sum(quot_total) as s  " +
	   			" FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	    ...
	    <script type="text/javascript">
	      ...
	      function drawChart() {
	        var data = google.visualization.arrayToDataTable([
	          ['Estatus', 'Oportunidades por Estatus'],
	       <% 
	       		String statusCaptionDonutChart = "";
	       		while(pmConnChart.next()) {
	       			bmoOpportunity.getStatus().setValue(pmConnChart.getString("opportunities", "stat"));
	       %>
	       		<%= "['" + bmoOpportunity.getStatus().getSelectedOption().getLabel() + "', " + pmConnChart.getInt("c") + "]," %>          
	       <%
	       		}
	       %>
	        ]);
				...
				String statusCaptionTypeChart = "";
	          	pmConnChart.beforeFirst();
	       		while(pmConnChart.next()) {
	          		bmoOpportunity.getStatus().setValue(pmConnChart.getString("opportunities", "stat"));
	       %>
	       		<%= "['" + bmoOpportunity.getStatus().getSelectedOption().getLabel() + "', " + pmConnChart.getDouble("s") + "]," %>          
	       <%
	       		}
	       		pmConnChart.close();
	...
	<table class="report" width="100%">
	    ...
	    // Agrupa oportunidades por el estatus de x moneda
	    String sqlGroup = "SELECT distinct(oppo_status) FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "venues")+" ON (venu_venueid = oppo_venueid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "users")+" ON (user_userid = oppo_userid) " +
	    ...
	    sqlGroup += " ORDER BY oppo_status ASC ";
    	pmConnGroup.doFetch(sqlGroup);
	        ...
	        while(pmConnGroup.next()) {
		    	...
		    	sql = " SELECT * FROM " + SQLUtil.formatKind(sFParams, "opportunities") +
		    			...
		    			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		    			" WHERE " +
		    			" oppo_status LIKE '" + pmConnGroup.getString ("opportunities", "oppo_status") + "'" +
			   			" AND oppo_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
		    			where;
		    			if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
				    		sql += " AND quot_quoteid IN ( " +
				...
				//System.out.println("consulta*: "+sql);
		          ...
		          while(pmConnList.next()) {  
			            ...
			            taxTotal += tax;
		          		...
		          		if(!(pmConnGroup.getString("opportunities", "oppo_status")).equals(oppoStatus)) {
		          			oppoStatus = pmConnGroup.getString("opportunities", "oppo_status");
	    ...
	    <script type="text/javascript">
	      ...
	      function drawChart() {
	       ...
	       <% 
	          ...
	          sql = " SELECT distinct(oppo_status) as stat, count(oppo_opportunityid) as c " +
	  	   			" FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	  	   	...
	  	   	sql += " GROUP BY stat ORDER BY stat";
	  	   	//System.out.println("sql_grafica_solo1: "+sql);
	  	   	pmConnChartGroup.doFetch(sql);
	  	   	
	  	   	while (pmConnChartGroup.next()) {
	  	   	
	       			bmoOpportunity.getStatus().setValue(pmConnChartGroup.getString("opportunities", "stat"));
	       %>
	       		<%= "['" + bmoOpportunity.getStatus().getSelectedOption().getLabel() + "', " + pmConnChartGroup.getInt("c") + "]," %>          
	       <%
	       		}
	       %>
	        ]);
	 	...
	 	<script type="text/javascript">
	      ...
	      function drawChart() {
			...
			String statusCaptionTypeChart = "";
	          sql = " SELECT distinct(oppo_status) as stat " +
	        		  " FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	          ...
	          while (pmConnChartGroup.next()) {
	        	  sql = " SELECT quot_total, quot_currencyid, quot_currencyparity " +
	        			  ...
	        			  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
	        			  " WHERE wfca_programid = " + programId + 
	        			  " AND oppo_status LIKE '" + pmConnChartGroup.getString("stat") + "'" +
	        			  groupFilter + where;
	        	  if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
	        		  sql += " AND quot_quoteid IN ( " +
	        				  " SELECT quot_quoteid  FROM " + SQLUtil.formatKind(sFParams, "quotes") +
	        	  ...
	        	  bmoOpportunity.getStatus().setValue(pmConnChartGroup.getString("opportunities", "stat"));
	       %>
	       		<%= "['" + bmoOpportunity.getStatus().getSelectedOption().getLabel() + "', " + totalTotalChart + "]," %>          
	       <%
	       		}
	       		pmConnChart.close();
