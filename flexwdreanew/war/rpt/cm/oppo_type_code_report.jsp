...
<%
	...
	String title = "Oportunidades Agrupado por flujo comercial";
	...
	PmOrderType pmOrderType = new PmOrderType(sFParams);
   	...
   	String sql = "", where = "", filters = "", groupFilter = "", sqlCurrency = "", whereProduct = "", whereProductFamily = "", whereProductGroup = "", whereSaleDate = "";
   	String startDate = "", endDate = "", status = "", productFamilyId = "", productGroupId = "", customer = "", wflowFunelId = "",saleEndDate = "", saleStartDate = "";
   	int wflowTypeId = 0, orderTypeId = 0, productId = 0, venueId = 0, referralId = 0, wflowPhaseId = 0, programId = 0, userId = 0, customerId = 0,
    ...
    // Filtro agrupacion
	if (wflowTypeId > 0) {
		groupFilter = " AND wfty_wflowtypeid = " + wflowTypeId;
    ...
    if (customerId > 0) {
   	...
   	if (!saleEndDate.equals("")) {
   		...
   		filters += " <i>Fecha Cierre Fin: </i>" + saleStartDate + ", ";
   	}
   	if (!status.equals("")) {
   	...
   	if (!productFamilyId.equals("")) {
   		...
   		filters += "<i>Familia Prod.: </i>" + request.getParameter("prod_productfamilyidLabel") + ", ";
   	}
    ...
    if (currencyId > 0) {
		...
		sqlCurrency = " SELECT cure_currencyid, cure_code, cure_name, cure_parity " +
	   			...
	   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"categoryforecasts")+" on (cafo_categoryforecastid = oppo_categoryforecastid) " +
...
<html>
		...
		<table border="0" cellspacing="0" cellpading="0" width="100%">
			...
			<tr>
				<td class="reportSubTitle">
					<b>Filtros:</b> <%= filters %>
					<br>
					<b>Agrupado por:</b> <%= ((!(currencyId > 0)) ? "Moneda, Tipo de Flujo" : "Tipo de Flujo")%>
					<b>Ordenado por:</b> Nombre de Tipo de Flujo, Clave Oportunidad
				</td>
	...
	//if que muestra el mensajede error
		...
		if (!(currencyId > 0)) {
			...
			sql = " SELECT wfty_name, count(oppo_opportunityid) as c, sum(quot_total) as s  " +
		   			" FROM " + SQLUtil.formatKind(sFParams, " opportunities ") + 
		   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" on (ortp_ordertypeid = oppo_ordertypeid) " +
		   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"categoryforecasts")+" on (cafo_categoryforecastid = oppo_categoryforecastid) " +
		   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
		   			...
		   			groupFilter + where + whereSaleDate;
		   	if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
		   		sql += " AND quot_quoteid IN ( " +
		   				" SELECT quot_quoteid  FROM " + SQLUtil.formatKind(sFParams, " quotes ") +
		   				" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"quotegroups")+" on (qogr_quoteid = quot_quoteid) " +
		    ...
		    <script type="text/javascript">
		      ...
		      function drawChart() {
		       ...
		       %>
		        ...
		        var options = {
		          title: ' Porcentaje por Tipo de Flujo',
		          pieHole: 0.4,
		        };
				...
				<% 
					String statusCaptionTypeChart = "";
		          	pmConnChart.beforeFirst();
		       		while(pmConnChart.next()) {
		       %>
		       			<%= "['" + pmConnChart.getString("wflowtypes", "wfty_name") + "', " + pmConnChart.getDouble("s") + "]," %>          
		       <%
		       		}
		       		pmConnChart.close();
		       %>
		...
		<table class="report" width="100%">
		    ...
		    String sqlGroup = "SELECT wfty_wflowtypeid, wfty_name FROM " + SQLUtil.formatKind(sFParams, " opportunities  ") + 
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"categoryforecasts")+" on (cafo_categoryforecastid = oppo_categoryforecastid) " +
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"venues")+" ON (venu_venueid = oppo_venueid) " +
		    		...
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"wflowphases")+" ON (wfph_wflowphaseid = wflw_wflowphaseid) " +	 
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		    		" WHERE wfca_programid = " + programId + whereSaleDate +
		    		" AND oppo_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
		    		groupFilter; 
		    if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
		    ...
		    sqlGroup += " GROUP BY wfty_name ORDER BY wfty_name ASC ";
	    	pmConnGroup.doFetch(sqlGroup);
		        ...
		        while(pmConnGroup.next()) {
			    	...
			    	sql = " SELECT * FROM " + SQLUtil.formatKind(sFParams, "opportunities ") +
			    			...
			    			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"wflowcategories")+" on (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
			    			" WHERE " +
			    			" oppo_wflowtypeid = " + pmConnGroup.getInt("wflowtypes", "wfty_wflowtypeid") +
				   			" AND oppo_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") + whereSaleDate +
			    			where;
			    			if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
					    		sql += " AND quot_quoteid IN ( " +
					    				" SELECT quot_quoteid  FROM " + SQLUtil.formatKind(sFParams, " quotes ") +
					...
					//System.out.println("consulta*: "+sql);
			          		...
			          		if(pmConnGroup.getInt("wflowtypes", "wfty_wflowtypeid") != wflowtypeid) {
						    	...
						    	<tr  class="reportGroupCell" >
						 			<td colspan="<%= 17 + dynamicColspan%>" class="reportGroupCell">
						    	  		<%= HtmlUtil.stringToHtml(pmConnGroup.getString("wflowtypes", "wfty_name")) %>
									</td>
								</tr>
							    <tr class="">
									...
									<% 	}%>
							     	<td class="reportHeaderCell"><%= sFParams.getFieldFormTitle(bmoOpportunity.getProgramCode(), bmoOpportunity.getFiscalPeriod()) %></td>
							     	<td class="reportHeaderCell"><%= sFParams.getFieldFormTitle(bmoOpportunity.getProgramCode(), bmoOpportunity.getFiscalYear()) %></td>
							     	<td class="reportHeaderCell"><%= sFParams.getFieldFormTitle(bmoOpportunity.getProgramCode(), bmoOpportunity.getAmountLicense()) %></td>
							     	<td class="reportHeaderCell"><%= sFParams.getFieldFormTitle(bmoOpportunity.getProgramCode(), bmoOpportunity.getAmountService()) %></td>
							     	<td class="reportHeaderCell"><%= sFParams.getFieldFormTitle(bmoOpportunity.getProgramCode(), bmoOpportunity.getCustomField1()) %></td>
							     	<td class="reportHeaderCell"><%= sFParams.getFieldFormTitle(bmoOpportunity.getProgramCode(), bmoOpportunity.getCategoryForecastId()) %></td>
			          ...
			          <tr class="reportCellEven">
							...
							<% 	}%>
						    <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("opportunities", "oppo_fiscalperiod"), BmFieldType.STRING)) %>
						    <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("opportunities", "oppo_fiscalyear"), BmFieldType.STRING)) %>
						    <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("opportunities", "oppo_amountlicense"), BmFieldType.STRING)) %>
						    <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("opportunities", "oppo_amountservice"), BmFieldType.STRING)) %>
						    <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("opportunities", "oppo_customfield1"), BmFieldType.STRING)) %>
						    <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("categoryforecasts","cafo_name"), BmFieldType.STRING)) %>
					        ...
					        if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
							    ...
							    sql = " SELECT prod_code, prod_name, prod_description,cust_code, cust_displayname, " +
					        		    ...
					        		    " AND qoit_productid > 0 " + whereSaleDate +
				       ...
				       if (amountSum > 0 || discountSum > 0 || taxSum > 0 || totalSum > 0) {
					     %>
						     <tr class="reportCellEven reportCellCode">
							     <td colspan="<%= (15 + dynamicColspan) - dynamicColspanMinus%>" align="right"> 
							     	&nbsp;
							     </td>
							     <% 
							     	if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
		...
		<%
		    ...
		    <script type="text/javascript">
		      ...
		      function drawChart() {
		       ...
		       <% 
		          ...
		          sql = " SELECT wfty_name, count(oppo_opportunityid) as c " +
		  	   			" FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
		  	   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
		  	   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"categoryforecasts")+" on (cafo_categoryforecastid = oppo_categoryforecastid) " +
		  	   			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
		  	   	...
		  	   	sql += " GROUP BY wfty_name ORDER BY wfty_name";
		  	   	//System.out.println("sql_grafica_solo1: "+sql);
		  	   	pmConnChartGroup.doFetch(sql);
		  	   	
		  	   	while (pmConnChartGroup.next()) {
	  	   		%>
		       		<%= "['" + pmConnChartGroup.getString("wflowtypes", "wfty_name") + "', " + pmConnChartGroup.getInt("c") + "]," %>          
	       		<%
		       		}
		  	   	%>
		        ]);
		 	...
		 	<script type="text/javascript">
		      ...
		      function drawChart() {
				...
				String statusCaptionTypeChart = "";
		          sql = " SELECT wfty_wflowtypeid, wfty_name " +
		        		  " FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
		        		  " LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
		        		  " LEFT JOIN "+ SQLUtil.formatKind(sFParams,"categoryforecasts")+" on (cafo_categoryforecastid = oppo_categoryforecastid) " +
		        		  " LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
		          ...
		          while (pmConnChartGroup.next()) {
		        	  sql = " SELECT quot_total, quot_currencyid, quot_currencyparity " +
		        			  ...
		        			  " LEFT JOIN "+ SQLUtil.formatKind(sFParams,"wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		        			  " WHERE wfca_programid = " + programId + 
		        			  " AND oppo_wflowtypeid = " + pmConnChartGroup.getInt("wflowtypes", "wfty_wflowtypeid") + whereSaleDate +
		        			  groupFilter + where;
		        	  if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
		        		  sql += " AND quot_quoteid IN ( " +
		        				  " SELECT quot_quoteid  FROM " + SQLUtil.formatKind(sFParams, "quotes") +
			...
			<table width="100%">
				<tr>
					<td width="50%"> <div id="donutchart" align="left" style="width: 100%; height: 400px;"></div> </td>
					<td width="50%"> <div id="typechart" align="right" style="width: 100%; height: 400px;"></div> </td>
				</tr>
			</table>
		<table class="report" width="100%">
	    <%
		    ...
		    String sqlGroup = "SELECT distinct(wfty_name), wfty_wflowtypeid FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"categoryforecasts")+" on (cafo_categoryforecastid = oppo_categoryforecastid) " +
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"customers")+" ON (cust_customerid = oppo_customerid) " +
		    		" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"venues")+" ON (venu_venueid = oppo_venueid) " +
		    ...
		    sqlGroup += " ORDER BY wfty_name ASC";
	    	pmConnGroup.doFetch(sqlGroup);
		        ...
		        while(pmConnGroup.next()) {
			    	...
			    	sql = " SELECT * FROM " + SQLUtil.formatKind(sFParams, "opportunities") +
			    			...
			    			" LEFT JOIN "+ SQLUtil.formatKind(sFParams,"wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
			    			" WHERE " +
			    			" oppo_wflowtypeid = " + pmConnGroup.getInt("wflowtypes", "wfty_wflowtypeid") + whereSaleDate +
			    			where;
			    			if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
					    		sql += " AND quot_quoteid IN ( " +
					    				" SELECT quot_quoteid  FROM " + SQLUtil.formatKind(sFParams, "quotes") +
