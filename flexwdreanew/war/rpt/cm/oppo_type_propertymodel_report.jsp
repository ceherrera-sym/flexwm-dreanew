...
<%
	// Inicializar variables
 	String title = "Oportunidades por Tipo";
...
//   	}
   	...
   	if (sFParams.restrictData(bmoOpportunity.getProgramCode()) || (userId > 0)) {
		...
		// Filtro por asignacion de Oportunidades
	...
	*/
    ...
    if (disclosureFilters.length() > 0)
   	...
   	sql = " SELECT wflowtypes.wfty_name, count(oppo_opportunityid) AS c FROM " + SQLUtil.formatKind(sFParams, " opportunities ") +
			 ...
			 groupFilter + where + 
			 " GROUP BY wflowtypes.wfty_name";
			 
	// SQL para el tipo de oportunidades
	String sqlTypes = " SELECT wflowtypes.wfty_name, sum(oppo_amount) as s, sum(quot_total) as q FROM " + SQLUtil.formatKind(sFParams, " opportunities ") +
			 " LEFT JOIN"+ SQLUtil.formatKind(sFParams," customers")+" ON (cust_customerid = oppo_customerid) " +
			 " LEFT JOIN"+ SQLUtil.formatKind(sFParams," referrals")+" ON (refe_referralid = cust_referralid) " +
	...
	System.out.println("sqlTypes: "+sqlTypes);
	...
	PmConn pmConnType = new PmConn(sFParams);
	pmConnType.open();
	pmConnType.doFetch(sqlTypes);
	...
	sql=(" SELECT COUNT(*) as contador FROM " + SQLUtil.formatKind(sFParams, " opportunities  ") + 
   		 ...
   		 " LEFT JOIN"+ SQLUtil.formatKind(sFParams," users")+" u2 ON(u2.user_userid = u1.user_parentid) " +
		     ...
		     " LEFT JOIN"+ SQLUtil.formatKind(sFParams," wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		     " WHERE wfca_programid = " + programId + 
			 where + groupFilter + " GROUP BY wfty_wflowtypeid ORDER BY wfty_wflowtypeid ASC");
 	...
 	<script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Tipo', 'Valor'],
		<% 
       		while(pmConnType.next()) {
       			String type = pmConnType.getString("wflowtypes", "wfty_name");
       			if (!(type.length() > 0)) type = "Indefinido";
       %>
       		<%//= "['" + type + ": Estimado" + "', " + pmConnType.getInt("s") + "]," %>
       		<%= "['" + type + ": Valor Total" + "', " + pmConnType.getDouble("q") + "]," %>
       <%
       		}
       %>
        ]);
...
// 					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customerphones")+" ON (cuph_customerid = cust_customerid) " +	 
	    ...
	    %>
	          ...
	          <tr class="reportCellEven">
	                ...
	                <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("wflowtypes", "wfty_name"), BmFieldType.CODE)) %>

	                <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConnList.getString("wflowphases", "wfph_name"), BmFieldType.CODE)) %>
