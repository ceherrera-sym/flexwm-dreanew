...
<%
	// Inicializar variables
 	String title = "Oportunidades Agrupadas por Vendedor";
	...
	PmOrderType pmOrderType = new PmOrderType(sFParams);
   	...
   	String sql = "", where = "", groupFilter = "", filters = "", sqlCurrency = "", whereProduct = "", whereProductFamily = "", whereProductGroup = "",saleStartDate = "";
   	String startDate = "", endDate = "",status = "", productFamilyId = "", productGroupId = "", customer = "", wflowFunelId = "",saleEndDate = "";
	...
	// Filtro agrupacion
	if (userId > 0) {
		groupFilter = " AND oppo_userid = " + userId;
		/*
    ...
    if (currencyId > 0) {
   		...
   		filters += "<i>Moneda: </i>" + request.getParameter("oppo_currencyidLabel") + ", <i>Paridad Actual : </i>" + defaultParity;
   	} else {
  ...
  //abro conexion para inciar el conteo consulta general
	...
	double sumTotal = 0, subTotalSumTotal = 0, descuentoSumTotal = 0, ivaSumTotal = 0;
...
if(sFParams.hasPrint(bmoProgram.getCode().toString()) || !(exportExcel.equals("1"))){ %>
	...
	<table border="0" cellspacing="0" cellpading="0" width="100%">
		...
		<tr>
			<td class="reportSubTitle">
				...
				<b>Ordenado por:</b> Clave Vendedor, Clave Oportunidad
			</td>
	...
	<%
		 ...
		 sql = " SELECT COUNT(*) as contador FROM " + SQLUtil.formatKind(sFParams, "opportunities") +
	    			...
	    			groupFilter +
//			   		" AND (oppo_userid = " + ((userId > 0) ? "" + userId : "" + pmConnGroup.getInt("users", "user_userid")) +
//	     				" OR oppo_wflowid IN ( " +
//							" SELECT wflw_wflowid FROM " + SQLUtil.formatKind(sFParams, " wflowusers  ") +
//							" LEFT JOIN " + SQLUtil.formatKind(sFParams, " wflows")+" ON (wflu_wflowid = wflw_wflowid) " +
//							" WHERE wflu_userid = " + ((userId > 0) ? "" + userId : "" + pmConnGroup.getInt("users", "user_userid")) +
//							" AND wflw_callercode = '" + bmoOpportunity.getProgramCode().toString() + "' " + 
//							" ) " + 
//					" )" + 
	    			where;
	...
	if (!(currencyId > 0)) {
		...
		sql = " SELECT user_userid, user_code, count(oppo_opportunityid) as c, sum(quot_total) as s  " +
	   			" FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" on (ortp_ordertypeid = oppo_ordertypeid) " +
	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	    ...
	    <script type="text/javascript">
	      ...
	      function drawChart() {
	        var data = google.visualization.arrayToDataTable([
	          ['Vendedor', 'Oportunidades por Vendedor'],
	       <% 
	       		String statusCaptionDonutChart = "";
	       		while(pmConnChart.next()) {
	       %>
	       			<%= "['" + pmConnChart.getString("users", "user_code") + "', " + pmConnChart.getInt("c") + "]," %>          
	       <%
	       		}
	       %>
	        ]);
			...
			<% 
				String statusCaptionTypeChart = "";
	          	pmConnChart.beforeFirst();
	       		while(pmConnChart.next()) {
	       %>
	       			<%= "['" + pmConnChart.getString("users", "user_code") + "', " + pmConnChart.getDouble("s") + "]," %>          
	       <%
	       		}
	       		pmConnChart.close();
	       %>
	...
	<table class="report" width="100%">
	    ...
	    // Agrupa oportunidades por vendedor de x moneda
	    String sqlGroup = "SELECT user_userid, user_code, user_email, user_firstname, user_fatherlastname, user_motherlastname " +
	    		"FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "venues")+" ON (venu_venueid = oppo_venueid) " +
	    ...
	    sqlGroup += " GROUP BY user_userid ORDER BY user_code ASC ";
    	pmConnGroup.doFetch(sqlGroup);
	        ...
	        while(pmConnGroup.next()) {
		    	...
		    	sql = " SELECT * FROM " + SQLUtil.formatKind(sFParams, "opportunities") +
		    			...
		    			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowphases")+" ON (wfph_wflowphaseid = wflw_wflowphaseid) " +	 
		    			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		    			" WHERE oppo_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
		   				" AND oppo_userid = " + pmConnGroup.getInt("users", "user_userid") +
// 				   		" AND (oppo_userid = " + ((userId > 0) ? "" + userId : "" + pmConnGroup.getInt("users", "user_userid")) +
// 		     				" OR oppo_wflowid IN ( " +
// 								" SELECT wflw_wflowid FROM " + SQLUtil.formatKind(sFParams, "wflowusers") +
// 								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " wflows")+" on (wflu_wflowid = wflw_wflowid) " +
...
// 						" )" + 
			            ...
			            taxTotal += tax;
		          		...
		          		if(pmConnGroup.getInt("users", "user_userid") != userid) {
						    ...
						    <tr class="">
						        ...
						        <%	if (sFParams.isFieldEnabled(bmoQuote.getCoverageParity())) { %>
										<td class="reportHeaderCellCenter">Cobertura T.C.</td>
						        <%	}%>
	...
	<%
	    ...
	    <script type="text/javascript">
	      ...
	      function drawChart() {
	       ...
	       <% 
	       		String statusCaptionDonutChart = "";
	          // Contar  las oportunidades por vendedor para la grafica DONUT
	          sql = " SELECT user_userid, user_code, count(oppo_opportunityid) as c " +
	  	   			" FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "users")+" ON (user_userid = oppo_userid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "quotes")+" ON (oppo_quoteid = quot_quoteid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "opportunitydetails")+" on (opde_opportunityid = oppo_opportunityid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "venues")+" ON (venu_venueid = oppo_venueid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "currencies")+" ON (cure_currencyid = quot_currencyid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflows")+" ON (wflw_wflowid = oppo_wflowid) " +
	  	   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowfunnels")+" ON (wflf_wflowfunnelid = wflw_wflowfunnelid) " +
	  	   	...
	  	   	sql += " GROUP BY user_userid ORDER BY user_code";
	  	   	//System.out.println("sql_grafica_solo1: "+sql);
	  	   	pmConnChartGroup.doFetch(sql);
	  	   	
	  	   	while (pmConnChartGroup.next()) {
  	   		%>
	       		<%= "['" + pmConnChartGroup.getString("users", "user_code") + "', " + pmConnChartGroup.getInt("c") + "]," %>          
       		<%
	       		}
	  	   	%>
	        ]);
	 	...
	 	<script type="text/javascript">
	      ...
	      function drawChart() {
			...
			String statusCaptionTypeChart = "";
	          sql = " SELECT user_userid, user_code " +
	        		  " FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	        		  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "referrals")+" ON (cust_referralid = refe_referralid) " +
	          ...
	          while (pmConnChartGroup.next()) {
	        	  sql = " SELECT quot_total, quot_currencyid, quot_currencyparity " +
	        			  ...
	        			  " LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
	        			  " WHERE wfca_programid = " + programId + 
	        			  " AND oppo_userid = " + pmConnChartGroup.getInt("users", "user_userid") + 
	        			  groupFilter + where;
	        	  if (productId > 0 || !productFamilyId.equals("") || !productGroupId.equals("")) {
	        		  sql += " AND quot_quoteid IN ( " +
	        				  " SELECT quot_quoteid  FROM " + SQLUtil.formatKind(sFParams, "quotes") +
    ...
    <%
	    ...
	    //filtra oportunidades por el vendedor
	    String sqlGroup = "SELECT distinct(user_userid), user_code, user_firstname, user_fatherlastname, user_motherlastname, user_email " +
	    		" FROM " + SQLUtil.formatKind(sFParams, "opportunities") + 
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "ordertypes")+" ON (ortp_ordertypeid = oppo_ordertypeid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "customers")+" ON (cust_customerid = oppo_customerid) " +
	    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, "venues")+" ON (venu_venueid = oppo_venueid) " +
	    ...
	    sqlGroup += " ORDER BY user_code ASC";
	    pmConnGroup.doFetch(sqlGroup);
	        ...
	        while(pmConnGroup.next()) {
		    	...
		    	sql = " SELECT * FROM " + SQLUtil.formatKind(sFParams, "opportunities") +
		    			...
		    			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowphases")+" ON (wfph_wflowphaseid = wflw_wflowphaseid) " +	 
		    			" LEFT JOIN " + SQLUtil.formatKind(sFParams, "wflowcategories")+" ON (wfca_wflowcategoryid = wfty_wflowcategoryid) " +
		    			" WHERE oppo_userid = " + pmConnGroup.getInt("users", "user_userid") +
// 				   		" AND (oppo_userid = " + ((userId > 0) ? "" + userId : "" + pmConnGroup.getInt("users", "user_userid")) +
// 		     				" OR oppo_wflowid IN ( " +
// 								" SELECT wflw_wflowid FROM " + SQLUtil.formatKind(sFParams, " wflowusers  ") +
// 								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " wflows")+" ON (wflu_wflowid = wflw_wflowid) " +
