...
<%
	// Inicializar variables
 	String title = "Proyectos Agrupados por Cliente";
	...
	PmProgram pmProgram  = new PmProgram(sFParams);
   	...
   	String sql = "", where = "", groupFilter = "", filters = "", status = "", paymentStatus = "", customer = "",
		...
		while (pmCurrencyWhile.next()) {
			...
			<table class="report" style="width: 100%;">
		    	...
		    	<script type="text/javascript">
			      	...
			      	function drawChart() {
			       	...
			       	sql = " SELECT cust_code, COUNT(proj_projectid) AS c, SUM(orde_total) AS s FROM projects "
							+ " LEFT JOIN orders ON (proj_orderid = orde_orderid) "
							...
							+ groupFilter 
							+ where
							+ " GROUP BY cust_code ORDER BY c DESC, cust_code ASC";
		    	...
		    	<script type="text/javascript"></script>
			    	<script type="text/javascript">
			      	...
			      	function drawChart() {
						...
						while (pmConnChartGroup.next()) {
							%>
			       			<%= "['" + pmConnChartGroup.getString("customers", "cust_code") + "', " +  pmConnChartGroup.getDouble("s") + "]," %>          
			   			<%
			   			}
			       		%>
			        	]);
			...
			<%
		    	double amountTotal = 0, totalTotal = 0, discountTotal = 0, taxTotal = 0, paymentsTotal = 0, balanceTotal = 0;
	   			...
	   			// Para este filtro de clientes considerar el id del modulo oportunidad
	   			
	   			pmConnGroup.doFetch(" SELECT cust_customerid, cust_code, cust_displayname, cust_legalname, cust_customertype FROM projects "
									+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) " 
								 	+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) " 
							     	+ " LEFT JOIN venues ON (venu_venueid = proj_venueid) "
							     	+ " LEFT JOIN cities ON (city_cityid = venu_cityid) "
	   			...
	   			while (pmConnGroup.next()) {
	   				int profileId = pmConnGroup.getInt("customers", "cust_customerid");
	...
	<%
    	...
    	<script type="text/javascript">
	      	...
	      	function drawChart() {
	       	...
	       	sql = " SELECT cust_code, COUNT(proj_projectid) AS c FROM projects "
					+ " LEFT JOIN orders ON (proj_orderid = orde_orderid) "
					+ " LEFT JOIN currencies ON (cure_currencyid = orde_currencyid) "
					+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) "
					+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) "
    	...
    	<script type="text/javascript"></script>
	    	<script type="text/javascript">
	      	...
	      	function drawChart() {
	        	var data = google.visualization.arrayToDataTable([
	          	['Cliente', 'Valor'],
	       		<%sql = " SELECT cust_customerid, cust_code FROM projects "
								+ " LEFT JOIN orders ON (proj_orderid = orde_orderid) "
								+ " LEFT JOIN currencies ON (cure_currencyid = orde_currencyid) "
								+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) "
								+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) "
								...
								+ groupFilter
								+ where 
								+ " GROUP BY cust_customerid ORDER BY cust_code";
				...
				while (pmConnChartGroup.next()) {
					...
					sql = " SELECT orde_currencyid, orde_currencyparity, orde_total FROM projects "
							...
							+ " LEFT JOIN users ON (user_userid = proj_userid) " 
							+ " WHERE wfca_programid = " + programId 
							+ " AND proj_customerid = " + pmConnChartGroup.getInt("customers", "cust_customerid") 
							+ where
							//+ groupFilter 
							+ " ORDER BY cust_code";
		...
		<%
	    	double amountTotal = 0, totalTotal = 0, discountTotal = 0, taxTotal = 0, paymentsTotal = 0, balanceTotal = 0;
   			...
   			// Para este filtro de cliente considerar el id del modulo oportunidad
   			pmConnGroup.doFetch(" SELECT cust_customerid, cust_code, cust_displayname, cust_legalname, cust_customertype FROM projects " 
					 			+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) " 
							 	+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) "    
						     	+ " LEFT JOIN venues ON (venu_venueid = proj_venueid) " 
   				...
   				while (pmConnList.next()) {
					...
					// Valores
   					...
   					if (pmConnGroup.getInt("customers", "cust_customerid") != customerid) {
	                	...
	                	if (pmConnGroup.getString("customers","cust_customertype").equals("" + BmoCustomer.TYPE_COMPANY)) {
	                		customer = pmConnGroup.getString("customers","cust_code") + " " +
	                				pmConnGroup.getString("customers","cust_legalname");
	                	} else { 
	                		customer = pmConnGroup.getString("customers","cust_code") + " " +
	                				pmConnGroup.getString("customers","cust_displayname");
	                	}
   						%>
						<tr>
							<td colspan="23" class="reportGroupCell">
