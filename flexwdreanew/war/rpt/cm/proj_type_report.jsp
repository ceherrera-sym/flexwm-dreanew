...
<%
	// Inicializar variables
 	String title = "Proyectos Agrupados por Tipo de Flujo";
	...
	PmProgram pmProgram  = new PmProgram(sFParams);
   	...
   	String sql = "", where = "", groupFilter = "", filters = "", status = "", paymentStatus = "", wflowtypes = "",
   			startDate = "", endDate = "", startDateCreate = "", endDateCreate = "", sqlCurrency = "";
   	int wflowTypeId = 0, projectId = 0, areaId = 0, wflowCategoryId = 0, venueId = 0, wflowPhaseId = 0, userId = 0, customerId = 0, 
   			referralId = 0, programId = 0, currencyId = 0, dynamicColspan = 0, dynamicColspanMinus = 0;
	...
	/*
		if (sFParams.restrictData(bmoProject.getProgramCode())) {
			...
			where += " AND ( " + " proj_userid = " + userId + " OR proj_wflowid IN ( "
...
<table border="0" style="width: 100%;">
	...
	<tr>
		<td class="reportSubTitle">
			...
			<b>Ordenado por:</b> Fecha de Proyecto
		</td>
		...
		while (pmCurrencyWhile.next()) {
			...
			<table class="report" style="width: 100%;">
		    	...
		    	<script type="text/javascript">
			      	...
			      	function drawChart() {
			       	...
			       	sql = " SELECT wfty_name, COUNT(proj_projectid) AS c, SUM(orde_total) AS s FROM projects "
							+ " LEFT JOIN orders ON (proj_orderid = orde_orderid) "
							+ " LEFT JOIN currencies ON (cure_currencyid = orde_currencyid) "
							+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) "
							+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) "
		    	...
		    	<script type="text/javascript"></script>
			    	<script type="text/javascript">
			      	...
			      	function drawChart() {
						...
						while (pmConnChartGroup.next()) {
							%>
			       			<%= "['" + pmConnChartGroup.getString("wflowtypes", "wfty_name") + "', " +  pmConnChartGroup.getDouble("s") + "]," %>          
			   			<%
			   			}
			       		%>
			        	]);
				...
				<tr>
					...
					<td width="50%" align="left">
						<div id="typechart<%= pmCurrencyWhile.getString("cure_code")%>" align="right" style="width: 100%; height: 300px;"></div>
					</td>
				</tr>
			</table>
			...
			<%
		    	double amountTotal = 0, totalTotal = 0, discountTotal = 0, taxTotal = 0, paymentsTotal = 0, balanceTotal = 0;
	   			...
	   			pmConnGroup.doFetch(
	   					"SELECT * FROM wflowtypes LEFT JOIN wflowcategories ON (wfty_wflowcategoryid = wfca_wflowcategoryid) "
	   							+ " WHERE wfca_programid = " + programId + groupFilter + "");

	   			int i = 1, y = 0;
	   			while (pmConnGroup.next()) {
	   				int wFlowTypeId = pmConnGroup.getInt("wflowtypes", "wfty_wflowtypeid");
	   				...
	   				sql = " SELECT * FROM projects "
	   						...
	   						+ " WHERE wfca_programid = " + programId
	   						+ " AND orde_currencyid =  " + currencyId
	   						+ " AND wflw_wflowtypeid = " + wFlowTypeId 
	   						+ where
	   						+ " ORDER BY proj_startdate ASC";
	   				pmConnList.doFetch(sql);

	   				double amountSum = 0, totalSum = 0, discountSum = 0, taxSum = 0, paymentsSum = 0,
	   						balanceSum = 0;
	   				i = 1;
...
// 	   					total = pmCurrencyExchange.currencyExchange(total, currencyIdOrigin, parityOrigin, currencyIdDestiny, parityDestiny);
	   					...
	   					if (!(pmConnGroup.getString("wflowtypes", "wfty_name").equals(wflowtypes))) {
	   						wflowtypes = pmConnGroup.getString("wflowtypes", "wfty_name");
	   						%>
							<tr>
					...
					<% 	} %>
	         			<tr class="reportCellEven">
						    ...
						    <%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmConnList.getString("wfca_name")), BmFieldType.STRING) %>
	...
	<%
    	...
    	<script type="text/javascript">
	      	...
	      	function drawChart() {
	       	...
	       	sql = " SELECT wfty_name, COUNT(proj_projectid) as c FROM projects "
					+ " LEFT JOIN orders ON (proj_orderid = orde_orderid) "
					+ " LEFT JOIN currencies ON (cure_currencyid = orde_currencyid) "
					+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) "
					+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) "
    	...
    	<script type="text/javascript"></script>
	    	<script type="text/javascript">
	      	...
	      	function drawChart() {
	        	var data = google.visualization.arrayToDataTable([
	          	['Tipo', 'Valor'],
	       		<%sql = " SELECT wfty_wflowtypeid, wfty_name FROM projects "
								+ " LEFT JOIN orders ON (proj_orderid = orde_orderid) "
								+ " LEFT JOIN currencies ON (cure_currencyid = orde_currencyid) "
								+ " LEFT JOIN customers ON (cust_customerid = proj_customerid) "
								+ " LEFT JOIN referrals ON (refe_referralid = cust_referralid) "
								...
								+ groupFilter
								+ where 
								+ " GROUP BY wfty_name ORDER BY wfty_name";
				...
				while (pmConnChartGroup.next()) {
					...
					sql = " SELECT orde_currencyid, orde_currencyparity, orde_total FROM projects "
							...
							+ " LEFT JOIN users ON (user_userid = proj_userid) " 
							+ " WHERE wfca_programid = " + programId 
							+ " AND proj_wflowtypeid = " + pmConnChartGroup.getInt("wflowtypes", "wfty_wflowtypeid") 
							+ where
							//+ groupFilter 
							+ " ORDER BY wfty_name";
