...
<%
	// Inicializar variables
 	String title = "Reporte de Ventas Cuenta Cliente";
...
// 	}
 ...
 //fin
  ...
  //abro conexion para inciar el conteo consulta general
	...
	PmConn pmConnWfsp = new PmConn(sFParams);
	pmConnWfsp.open();
...
<body class="default" <%= permissionPrint %> style="padding-right: 10px">
	...
	<table class="report" border="0" width="100%">
		<tr>
			...
			<!-- Dias y CxC-->
			<th class="reportHeaderCellCenter" width="2%" colspan="2">D&iacute;as</th>
			...
			<th class="reportHeaderCellCenter" width="36%" colspan="6">CxC</th>
		<tr>
		<tr>
			<!-- General 2da linea-->
			<th class="reportHeaderCell" width="3%">&nbsp;</th>
			<th class="reportHeaderCell" width="14%">&nbsp;</th>
			<th class="reportHeaderCell" width="20%">&nbsp;</th>
			<th class="reportHeaderCell" width="7%">&nbsp;</th>
			<th class="reportHeaderCell" width="5%">&nbsp;</th>
			<!-- Dias y CxC 2da linea-->
			<th class="reportHeaderCell" width="1%">&nbsp;En paso actual</th>
			<th class="reportHeaderCell" width="1%">&nbsp;Fecha Avance</th>
			<th class="reportHeaderCell" width="3%">&nbsp;</th>
			<th class="reportHeaderCell" width="3%">&nbsp;</th>
			<th class="reportHeaderCell" width="5%">&nbsp;</th>

			<th class="reportHeaderCell" width="8%">Concepto</th>
			<th class="reportHeaderCell" width="5%">F. Prog.</th>
			<th class="reportHeaderCell" width="5%">Estatus</th>
			<th class="reportHeaderCellRight" width="6%">Monto</th>
...
%>
		...
		<tr class="reportCellEven">
			...
			<%
				...
				String daysEndDate = SFServerUtil.nowToString(sFParams, "yyyy-MM-dd");

				
				String sqlStep;
				sqlStep= " SELECT * FROM " + SQLUtil.formatKind(sFParams, " wflowsteps ") +
							" WHERE wfsp_wflowid = " + pmPropertySale.getInt("propertysales", "prsa_wflowid") +
							" AND wfsp_enabled = 1";
				
				pmConnWfsp.doFetch(sqlStep);
				while(pmConnWfsp.next()){
					if(pmConnWfsp.getString("wfsp_enddate").length() > 0)
							daysEndDate = pmConnWfsp.getString("wfsp_enddate").substring(0,10);
					else 	daysEndDate = SFServerUtil.nowToString(sFParams, "yyyy-MM-dd");

					daysStartDate = pmConnWfsp.getString("wfsp_startdate").substring(0,10);
				}	
				
				//System.out.println("sql: "+sqlStep);
				//System.out.println("f.ini: "+daysStartDate.substring(0,10) + "  f.end: "+daysEndDate.substring(0,10));
				
				int diff = FlexUtil.daysDiff(sFParams, daysStartDate, daysEndDate);
			%>
			<td class="reportCellText" rowspan="<%= count%>"><%= diff%></td>
			<td class="reportCellText" rowspan="<%= count%>"><%= SFServerUtil.addDays(sFParams.getDateFormat(), SFServerUtil.nowToString(sFParams, "yyyy-MM-dd"), -diff)%>
			</td>
		    ...
		    double totalSumPropertySale = 0, paymentsSumPropertySale = 0, balanceSumPropertySale = 0;
	    		while(pmConnRaccW.next()) {
	    			bmoRaccount.getStatus().setValue(pmConnRaccW.getString("raccounts", "racc_status"));
	    			// la cxc de Reembolso 
			...
			<td class="reportCellText"><%= HtmlUtil.stringToHtml(pmConnRaccW.getString("racc_invoiceno") + ": " + pmConnRaccW.getString("racc_description"))%>
			</td>
			<td class="reportCellText"><%= HtmlUtil.stringToHtml(pmConnRaccW.getString("racc_duedate"))%>
			</td>
			<td class="reportCellText"><%= HtmlUtil.stringToHtml(bmoRaccount.getStatus().getSelectedOption().getLabel().toString())%>
			</td>
			<td class="reportCellCurrency" align="right"><%= formatCurrency.format(pmConnRaccW.getDouble("racc_total"))%>
			</td>
			<td class="reportCellCurrency" align="right"><%= formatCurrency.format(pmConnRaccW.getDouble("racc_payments"))%>
			</td>
			<td class="reportCellCurrency" align="right"><%= formatCurrency.format(pmConnRaccW.getDouble("racc_balance"))%>
			</td>
		</tr>
		<%
