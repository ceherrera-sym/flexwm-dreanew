...
//   			System.out.println("contador DE REGISTROS --> "+count);
	    ...
	    sql = " SELECT prsa_propertysaleid,prsa_tags, prsa_code, prty_code,cust_phone,cust_mobile,cust_email,cust_code, cust_customertype, cust_nss, cust_displayname, cust_legalname, cust_datecreate, prsa_startdate, prsa_cancelldate, wfty_name,prsa_status, prsa_type, " +
			  ...
			  " prsa_description, wfsp_progress, deve_code, dvph_code, dvbl_code, prty_lot, prty_number, ptym_code, lomt_name, prsa_losecomments, orde_taxapplies, orde_paymentstatus, " +
			  " u1.user_code AS promo, u2.user_code AS coord, u3.user_code AS gerente, u4.user_code AS gerentecom, u5.user_code AS direc " +
    	...
    	sql = " SELECT count(contador) AS contador FROM(Select Count(*) AS contador FROM "+ SQLUtil.formatKind(sFParams," wflowsteps ") +
			...
			if(pmConnCount.next())
				count=pmConnCount.getInt("contador");
// 			System.out.println("contador DE REGISTROS --> "+count);
    	sql = " SELECT prsa_propertysaleid,prsa_tags,prsa_code, prty_code,cust_phone,cust_email,cust_mobile,cust_code, cust_customertype, cust_nss, cust_displayname, cust_legalname, cust_datecreate, prsa_startdate, prsa_cancelldate, wfty_name,prsa_status, prsa_type, " +
	    	  " dvbl_code, prty_street, prty_number, ptym_name, prsa_orderid, prty_habitability, prty_progress, prty_finishdate, orde_total, wflw_wflowid, wflw_hasdocuments, wfty_name, wfph_name, " +
			  " prsa_description, wfsp_progress, deve_code, dvph_code, dvbl_code, prty_lot, prty_number, ptym_code, lomt_name, prsa_losecomments, orde_taxapplies, orde_paymentstatus, " +
			  " u1.user_code AS promo, u2.user_code AS coord, u3.user_code AS gerente, u4.user_code AS gerentecom, u5.user_code AS direc, " +
...
<table class="report" border="0">
	<tr class="">		
		...
		<td class="reportHeaderCell">Datos Cliente</td>
		<td class="reportHeaderCell">F. Creaci&oacute;n CL.</td>
		<td class="reportHeaderCell">F. Vta.</td>
		<td class="reportHeaderCell">F.Cancelaci&oacute;n</td>
		<td class="reportHeaderCell">Motivo Canc.</td>
		<td class="reportHeaderCell">Comentario Canc.</td>
		...
		<td class="reportHeaderCell">Doc. Completos</td>
		<td class="reportHeaderCell">Notario</td>
		<td class="reportHeaderCell"># Escritura</td>
		
		<!-- Inmueble-->
		<td class="reportHeaderCell">Manzana</td>
...
%>
		...
		<!-- General-->
			...
			<td class="reportCellText">
				<%if(!(pmPropertySale.getString("customers","cust_phone").equals(""))){ %>
					<%=pmPropertySale.getString("customers","cust_phone") %>
				<%} %>
				<%if(!(pmPropertySale.getString("customers","cust_mobile").equals(""))){ %>
					<br>
					<%=pmPropertySale.getString("customers","cust_mobile") %>	
				<%} %>
				<%if(!(pmPropertySale.getString("customers","cust_email").equals(""))){ %>			
					<br>
					<%=pmPropertySale.getString("customers","cust_email") %>
				<%} %>
			</td>
			<%= HtmlUtil.formatReportCell(sFParams, pmPropertySale.getString("customers","cust_datecreate"), BmFieldType.DATE) %> 
			...
			<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmPropertySale.getString("losemotives","lomt_name"), BmFieldType.STRING)) %> 
			<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmPropertySale.getString("propertysales","prsa_losecomments"), BmFieldType.STRING)) %> 
		...
		<!-- Venta-->
			...
			<%= HtmlUtil.formatReportCell(sFParams, ((pmPropertySale.getBoolean("wflw_hasdocuments")) ? "Si" : "No"), BmFieldType.STRING) %> 
			<!-- inhabilitado-->
			...
			<%= HtmlUtil.formatReportCell(sFParams, writingNumber, BmFieldType.STRING) %>
...
// 			    }
			...
			<%  
				...
				String tagListHtml = "";
				if (!pmPropertySale.getString("prsa_tags").equals("")) {
					String[] split = pmPropertySale.getString("prsa_tags").split("\\" + SFParams.TAG_SEPARATOR);
					
					for (int j = 0; j < split.length; j++) {
						String tagId = split[j];
						if (!tagId.equals("")) {
