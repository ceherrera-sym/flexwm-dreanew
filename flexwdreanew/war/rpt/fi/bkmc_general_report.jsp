...
<%
	// Inicializar variables
 	String title = "Reporte Detalle de Mov. Bancarios";	
...
<%   
   	...
   	sql = " SELECT COUNT(*) AS contador FROM " + SQLUtil.formatKind(sFParams, "bankmovconcepts") +
        ...
        " LEFT JOIN " + SQLUtil.formatKind(sFParams, "requisitions") + " reqiBkmc ON (bkmc_requisitionid = reqiBkmc.reqi_requisitionid) " +
        " LEFT JOIN " + SQLUtil.formatKind(sFParams, "companies") + " ON (bkac.bkac_companyid = comp_companyid) " +
        ...
        " LEFT JOIN " + SQLUtil.formatKind(sFParams, "requisitions") + " reqiPacc ON (rerc_requisitionid = reqiPacc.reqi_requisitionid) " +
        " LEFT JOIN " + SQLUtil.formatKind(sFParams, "orders") + " ordeReqi ON (ordeReqi.orde_orderid = reqiPacc.reqi_orderid) " +
   	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "raccounts") + " ON (bkmc_raccountid = racc_raccountid) "+
   	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "orders") + " ordeRacc ON (ordeRacc.orde_orderid = racc_orderid)";	
   	 	if (enableBudgets) {
			sql += " LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitems") + " ON (bkmc_budgetitemid = bgit_budgetitemid) " +
   	 	...
   	 	sql += " WHERE bkmc_bankmovconceptid > 0 " + 
   		where ;
   		// Descartar anticipos a proveedores en la primera tabla
// 		" AND bkmv_bankmovementid NOT IN ( " +
//   				" SELECT bkmv_bankmovementid FROM " + SQLUtil.formatKind(sFParams, "bankmovements") + 
//   				" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitems") + " ON (bkmv_budgetitemid = bgit_budgetitemid) " +
//   	     	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitemtypes") + " ON (bgit_budgetitemtypeid = bgty_budgetitemtypeid) " +
//   	     	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgets") + " ON (bgit_budgetid = budg_budgetid) " +
//   	     	 	" WHERE bkmv_bankmovementid > 0 " +
//   	     	 	whereBudget + 
//   	     	 	whereBudgetItem +
...
// 		" ) " ;
   	if (enableBudgets) {
   		sql += whereBudget + 
	     	 	whereBudgetItem ;
//    		sql += 	" AND ( bkmc_bankmovconceptid IN ( " +
// 		   				" SELECT bkmc_bankmovconceptid FROM " + SQLUtil.formatKind(sFParams, "bankmovconcepts") +
// 		   			 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "bankmovements") + " ON (bkmc_bankmovementid = bkmv_bankmovementid)" +
// 		   	         	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "paccounts") +" ON (bkmc_paccountid = pacc_paccountid) " +
// 		   				" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitems") + " ON (pacc_budgetitemid = bgit_budgetitemid) " +
// 		   	     	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitemtypes") + " ON (bgit_budgetitemtypeid = bgty_budgetitemtypeid) " +
// 		   	     	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgets") + " ON (bgit_budgetid = budg_budgetid) " +
// 		   	     	 	" WHERE bkmc_bankmovconceptid > 0 " +
// 		   	     	 	whereBudget + 
// 		   	     	 	whereBudgetItem +
// 		   				" OR bkmc_bankmovconceptid IN ( " +
// 		   				" SELECT bkmc_bankmovconceptid FROM " + SQLUtil.formatKind(sFParams, "bankmovconcepts") +
// 		   			 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "bankmovements") + " ON (bkmc_bankmovementid = bkmv_bankmovementid)" +
// 		   	         	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "raccounts") +" ON (bkmc_raccountid = racc_raccountid) " +
// 		   				" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitems") + " ON (racc_budgetitemid = bgit_budgetitemid) " +
// 		   	     	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitemtypes") + " ON (bgit_budgetitemtypeid = bgty_budgetitemtypeid) " +
// 		   	     	 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgets") + " ON (bgit_budgetid = budg_budgetid) " +
// 		   	     	 	" WHERE bkmc_bankmovconceptid > 0 " +
// 		   	     	 	whereBudget + 
...
//    				" )";
   	...
   	sql += " ORDER BY bkmc_bankmovementid ";
				
		int count =0;
		// ejecuto el sql DEL CONTADOR 1 sql
...
// 			" ) " ;
	   	if (enableBudgets) {
	   		sql += whereBudget + 
	     	 	whereBudgetItem ;
	   		// Entraba a buscar la partida correspondiente
// 	   		sql += 	" AND ( bkmc_bankmovconceptid IN ( " +
// 		   				" SELECT bkmc_bankmovconceptid FROM " + SQLUtil.formatKind(sFParams, "bankmovconcepts") +
// 		   			 	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "bankmovements") + " ON (bkmc_bankmovementid = bkmv_bankmovementid)" +
// 		   	         	" LEFT JOIN " + SQLUtil.formatKind(sFParams, "paccounts") +" ON (bkmc_paccountid = pacc_paccountid) " +
...
// 	   			" )";
	   	...
	   	while (pmConn.next()) { %>   
			...
			<TR class="reportCellEven">
				...
				<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConn.getString("bkac", "bkac_name"), BmFieldType.STRING))%>				
				...
				<% } %>				
				<% if (pmConn.getInt("bkmc_requisitionid") > 0) { %>
					<%= HtmlUtil.formatReportCell(sFParams, pmConn.getString("reqiBkmc", "reqi_code"),BmFieldType.STRING)%>
				<% } else { %>
				...
				if (pmConn.getInt("bkmc_paccountid") > 0) {
					...
					%>
					<%= HtmlUtil.formatReportCell(sFParams, pmConn.getString("paccounts", "pacc_code") 
							+ " " + pmConn.getString("paccounts", "pacc_invoiceno"), BmFieldType.STRING)%>
				<% } else if (pmConn.getInt("bkmc_raccountid") > 0) { 
					ordeCode = pmConn.getString("ordeRacc.orde_code") + " " + pmConn.getString("ordeRacc.orde_name");
				%>	
					<%= HtmlUtil.formatReportCell(sFParams, pmConn.getString("raccounts", "racc_code")
							+ " " + pmConn.getString("raccounts", "racc_invoiceno"), BmFieldType.STRING)%>
				<% } else { %>
				...
				<% if (enableBudgets) {
					String budgetSql = "", budget = "", budgetItem = "";

					int budgetItemIdBkmc = 0;
					if (pmConn.getInt("reqiBkmc", "reqi_budgetitemid") > 0)
						budgetItemIdBkmc = pmConn.getInt("reqiBkmc", "reqi_budgetitemid");
					else if (pmConn.getInt("paccounts", "pacc_budgetitemid") > 0)
						budgetItemIdBkmc = pmConn.getInt("paccounts", "pacc_budgetitemid");
					else if (pmConn.getInt("raccounts", "racc_budgetitemid") > 0)
						budgetItemIdBkmc = pmConn.getInt("raccounts", "racc_budgetitemid");
					else 
						budgetItemIdBkmc = pmConn.getInt("bankmovements", "bkmv_budgetitemid");
					
					if (budgetItemIdBkmc > 0) {
						budgetSql = " SELECT budg_name, bgty_name FROM " + SQLUtil.formatKind(sFParams, "budgetitems") 
						+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgets") +" ON (bgit_budgetid = budg_budgetid) " 
						+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitemtypes") + " ON (bgit_budgetitemtypeid = bgty_budgetitemtypeid) " 
						+ " WHERE bgit_budgetitemid = " + budgetItemIdBkmc;
						
						pmConnBkmvTras.doFetch(budgetSql);
						if (pmConnBkmvTras.next()) {
							budget = pmConnBkmvTras.getString("budg_name");
							budgetItem = pmConnBkmvTras.getString("bgty_name");
						}
					}
				%> 
					<%= HtmlUtil.formatReportCell(sFParams, "" + budget, BmFieldType.STRING)%>				
					<%= HtmlUtil.formatReportCell(sFParams, "" + budgetItem, BmFieldType.STRING)%>
				<% } %>	
				...
				if (pmConn.getString("bankmovtypes","bkmt_type").equals("W")) {
					amountD = 0;
					amountW = pmConn.getDouble("bkmc_amount");
				} else {
					amountW = 0;	
					amountD = pmConn.getDouble("bkmc_amount");
				}	
				%>
...
<% 	// **************************************************** Anticipos **************************************************** %>
		<tr><td  colspan="<%= colspan%>">&nbsp;</td></tr>
		<tr><td class="reportGroupCell" colspan="<%= colspan%>">Movimientos sin Detalle</td></tr>
		<tr>
			<td class="reportHeaderCell">#</td>
			<td class="reportHeaderCell">Clave</td>
			<td class="reportHeaderCell">Nombre Cta.</td>
