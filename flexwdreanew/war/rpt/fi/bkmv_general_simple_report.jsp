...
<%
   ...
   while (pmBankMovAccounts.next()) {
	   ...
	   <tr class="reportCellEven">
		   <td class="reportGroupCell" colspan="13">
		    ...
		    <tr>
				...
				<td class="reportHeaderCell">No.Cheque</td>				
	   ...
	   sql = "SELECT * FROM " + SQLUtil.formatKind(sFParams, " bankmovements ") +
	         ...
	         " WHERE bkmv_bankaccountid = " + pmBankMovAccounts.getInt("bkac_bankaccountid") + 
	         where +
	         " ORDER BY bkmv_duedate, bkmv_nocheck, bkmv_bankmovementid ";
	   ...
	   while (pmBankMovements.next()) { %>	   
			...
			<TR class="reportCellEven">
				...
				<%= HtmlUtil.formatReportCell(sFParams, "" + pmBankMovements.getString("bankmovements", "bkmv_nocheck"), BmFieldType.STRING)%>
				...
				<% } else { 
					String description = pmBankMovements.getString("bkmv_description");
					if (pmBankMovements.getString("bankmovtypes", "bkmt_category").equals("" + BmoBankMovType.CATEGORY_TRANSFER)) {						
						//Obtener el Concepto
						sql = " SELECT bkmc_code FROM " + SQLUtil.formatKind(sFParams, " bankmovconcepts ") + 
						      " WHERE bkmc_bankmovementid = " + pmBankMovements.getInt("bkmv_bankmovementid");
						pmConn.doFetch(sql);
						if (pmConn.next()) description = pmConn.getString("bankmovconcepts", "bkmc_code");
						
					} 
				%>	
					<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, "" + description, BmFieldType.STRING)) %>
				<% } %>	
				<%//=HtmlUtil.formatReportCell(sFParams, pmBankMovements.getString("bankmovements", "bkmv_bankreference"), BmFieldType.STRING)%>				
				<%							
				...
				<%
				bankMov.getStatus().setValue(pmBankMovements.getString("bankmovements", "bkmv_status"));

				%>
            	<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, bankMov.getStatus().getSelectedOption().getLabel(), BmFieldType.STRING)) %>
