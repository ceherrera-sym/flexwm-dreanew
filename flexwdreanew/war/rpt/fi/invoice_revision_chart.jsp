	...
	pmConn.open();
 	...
 	// Asigna usuario seleccionado
   	...
   	for (int i = 1; i <= 12; i++) {
   		...
   		sql = "SELECT COUNT(racc_raccountid) as total FROM " + SQLUtil.formatKind(sFParams, "raccounts")
   			+ " WHERE racc_receivedate BETWEEN CAST('" + currentYear + "-" + i + "-01' AS DATE) "
   			+ " AND CAST('" + (currentYear) + "-" + i + "-30' AS DATE) AND racc_status like '"+BmoRaccount.STATUS_AUTHORIZED+"' "
   			+ where;  
   		...
   		sql = "SELECT count(racc_raccountid) as total FROM " + SQLUtil.formatKind(sFParams, "raccounts")
   			+ " WHERE racc_receivedate BETWEEN CAST('" + currentYear + "-" + i + "-01' AS DATE) "
   			+ " AND CAST('" + (currentYear) + "-" + i + "-30' AS DATE) AND racc_status like '"+BmoRaccount.STATUS_REVISION+"' "
   		   	+ where;  
	      ...
	      function drawChart() {
	        ...
	        var options = {
	          chart: {
	            ...
	            subtitle: 'Cuentas por pagar: <%= "Autorizadas " %>-<%= "Revisión" %>',
	          },
