...
<%
	// Inicializar variables
 	String title = "Reporte General de Cuentas por Pagar";
...
// 		sqlCurrency = "SELECT cure_currencyid, cure_name FROM " + SQLUtil.formatKind(sFParams, " currencies ");
		...
		sqlCurrency = " SELECT cure_currencyid, cure_name " + 
        ...
        if (enableBudgets) {
        	sqlCurrency += " LEFT JOIN " + SQLUtil.formatKind(sFParams, " budgetitems") + " ON (bgit_budgetitemid = pacc_budgetitemid)" +
        ...
        sqlCurrency += " WHERE pacc_paccountid > 0 " +          
        		where + 
...
//             while (pmSuppliers.next()) {
            	...
            	if (pmCurrencyWhile.getInt("currencies", "cure_currencyid") != currencyIdWhile) {
            		...
            		%>
            		<tr>
	            		<td class="reportHeaderCellCenter" colspan="<%= (22 + activeBudgets)%>">
                ...
                sql = "SELECT pacc_paccountid, pacc_code, pacc_amount, pacc_payments, pacc_tax, pacc_total, reqi_code, pacc_invoiceno, comp_name, pacc_description, pact_name, " +
                		...
                		" pacc_currencyparity, reqi_currencyparity, cure_code,pacc_file ,supl_name";
                if (enableBudgets) {
                ...
                if (enableBudgets) {
                	sql += " LEFT JOIN " + SQLUtil.formatKind(sFParams, " budgetitems")+" ON (bgit_budgetitemid = pacc_budgetitemid)" +
    	   	  		...
    	   	  		sql +=	" WHERE pacc_currencyid = " + pmCurrencyWhile.getInt("cure_currencyid") +
                		where + 
//                 		" AND pacc_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
                		" AND pacc_paccountid IN " +
                				" (SELECT bkmc_paccountid FROM bankmovconcepts " +
...
<!--            				</tr> -->
           				...
           				<% 	} %>
	           				...
	           				<td class="reportHeaderCell">Fecha de Pago</td>
...
<%--              <% 	} %> --%>
  		<% while (pmPaccounts.next()) {
  				...
  				String sqlPayment = "SELECT * FROM bankmovconcepts  LEFT JOIN bankmovements ON(bkmc_bankmovementid = bkmv_bankmovementid) WHERE bkmc_paccountid = " + pmPaccounts.getInt("pacc_paccountid") + " group by bkmc_bankmovementid";
 				
 				String datePayment = "";
 					pmSuppliers.doFetch(sqlPayment);
 					while(pmSuppliers.next()){
 						if(!datePayment.equals("")) datePayment += " , ";
 						datePayment += pmSuppliers.getString("bkmv_code")+  " : " + pmSuppliers.getString("bkmv_duedate");
//  					
 					}
 					sqlPayment = "SELECT * FROM paccountassignments LEFT JOIN paccounts ON(pacc_paccountid = pass_paccountid ) WHERE pass_foreignpaccountid = " + pmPaccounts.getInt("pacc_paccountid")  + " group by pass_paccountid ";
 					pmSuppliers.doFetch(sqlPayment);
 					while(pmSuppliers.next()){
 						if(!datePayment.equals("")) datePayment += " , ";
 						datePayment += " Nota de Credito " + pmSuppliers.getString("pacc_code")+  " : " + pmSuppliers.getString("pacc_paymentdate");
 					}%>
                  ...
                  <TR class="reportCellEven">
                       ...
                       <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmPaccounts.getString("paccounts","pacc_description"), BmFieldType.STRING)) %>
                       	<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmPaccounts.getString("suppliers","supl_name"), BmFieldType.STRING)) %>
                       ...
                       <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, bmoPaccount.getStatus().getSelectedOption().getLabel(), BmFieldType.STRING)) %>
                        <%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, datePayment, BmFieldType.STRING))%>
...
//             } //End while pmSuppliers 
        	...
        	%>    
        		<tr><td colspan="<%= (22 + activeBudgets)%>">&nbsp;</td></tr>
            	<tr class="reportCellEven reportCellCode">
...
//         while (pmSuppliers.next()) {
            ...
            int supplierid = 0;
            // Obtener CP
            sql = "SELECT pacc_paccountid, pacc_code, pacc_amount, pacc_payments, pacc_tax, pacc_total, reqi_code, pacc_invoiceno,pacc_paccountid, comp_name, pacc_description, pact_name, " +
            		" pacc_receivedate, pacc_duedate,pacc_status, pacc_paymentstatus, pacc_balance, pacc_requisitionid, pacc_currencyid, " +
            		" pacc_currencyparity, reqi_currencyparity, cure_code,pacc_file ,supl_name";
            if (enableBudgets) {
            	sql += ", budg_name, bgty_name, area_name ";
...
//             sql += " WHERE pacc_supplierid = " + pmSuppliers.getInt("supl_supplierid") +                                 
            	sql += " WHERE pacc_paccountid > 0  " + 	where + 
