...
<%
	// Inicializar variables
 	String title = "Reporte CxC Agrupadas por Cliente";
	...
	BmoCustomer bmoCustomer = new BmoCustomer();
   	...
   	String filters = "";
   	String budget = "",area = "", areaId = "", budgetItemId = "";
...
//   	}
    ...
    if (disclosureFilters.length() > 0)
   	...
   	PmConn pmBudgetOrder = new PmConn(sFParams);
   	pmBudgetOrder.open();
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			...
			<b>Ordenado por:</b> Clave Cliente, Clave CxC
		</td>
...
<table class="report" border="0">               
	...
	sql +=	" WHERE racc_raccountid > 0 " +
			//" AND ract_type = '" + BmoRaccountType.TYPE_WITHDRAW + "'" + 
			whereOrderId+
			where +
			whereFailure +
			whereLinked +
			" ORDER BY  racc_customerid ASC,"+ "orde_code ASC, "+ "racc_code ASC";
		int count =0;
	...
	if (!(currencyId > 0)) {
		...
		while (pmCurrencyWhile.next()) {			
			sql = " SELECT DISTINCT(racc_customerid) AS cliente," +
		    		" cust_customerid, cust_code, cust_legalname, cust_displayname,cust_customercategory " +
		    ...
		    while (pmCustomer.next()) {
		    	...
		    	sql +=	" WHERE racc_raccountid > 0 " +
		    			" AND racc_currencyid = " + pmCurrencyWhile.getInt("cure_currencyid") +
		    			//" AND ract_type = '" + BmoRaccountType.TYPE_WITHDRAW + "'" + 
		    			" AND racc_customerid = " + pmCustomer.getInt("cust_customerid") +
		    			whereOrderId +
		    			where +
		    			whereFailure +
		    			whereLinked +
		    			" ORDER BY  racc_customerid ASC,"+ "orde_code ASC, "+ "racc_raccountid ASC";
		    	
			   pmRaccounts.doFetch(sql);                           
		       while (pmRaccounts.next()) {	
		    	   budget= "";
		    	   area = "";
		    	   if(pmRaccounts.getInt("orders","orde_orderid") > 0){
		    	   	 ...
		    	   	 sql += " WHERE orde_orderid = "+ pmRaccounts.getInt("orders","orde_orderid");
		    	   			pmBudgetOrder.doFetch(sql);	    	   
		    	   		if(pmBudgetOrder.next()){
		    	   			if (enableBudgets) { 
		    		   		budget = pmBudgetOrder.getString("budgets","budg_name")+" : "+ pmBudgetOrder.getString("budgetitemtypes", "bgty_name");
		    		   		area = pmBudgetOrder.getString("areas","area_name");
		    	   			}
		    	   		}
		    	   }
		    	   sql = "";
		    	   if ((pmCustomer.getInt("cust_customerid") != customerid || primerRegistro == 0) ) {
			    		 customerid = pmCustomer.getInt("cust_customerid");
			    		 bmoCustomer.getCustomercategory().setValue(pmCustomer.getString("customers","cust_customercategory"));
...
//	 		               currencyIdOrigin = pmCustomer.getInt("orde_currencyid");
//	 		               parityOrigin = pmCustomer.getDouble("orde_currencyparity");
//	 		               currencyIdDestiny = pmCurrencyWhile.getInt("cure_currencyid");
//	 		               parityDestiny = pmCurrencyWhile.getDouble("cure_parity");
			               
			    	%>       
			               <tr>
				               <td class="reportGroupCell" colspan="24">
	<%-- 				               <%= HtmlUtil.stringToHtml(pmCustomer.getString("orde_name"))%> | --%>
					               Cliente:
					            	   	<%= pmCustomer.getString("cust_code")%>
					               		<%= HtmlUtil.stringToHtml(pmCustomer.getString("cust_displayname"))%> |
				            ...
				            <tr class="">
				               ...
				               <%	if (enableBudgets) { %>
				               <td class="reportHeaderCell">Partida Pres. CxC</td>
		               		   <td class="reportHeaderCell">Dpto. CxC</td>
		               		   <% } %>
				               <td class="reportHeaderCell">Pedido</td>	
				               <%	if (enableBudgets) { %>	
				               <td class="reportHeaderCell">Partida Pres. Ped</td>
		               		   <td class="reportHeaderCell">Dpto. Ped</td>	
		               		   <% } %>		               
...
//					   balanceRacc = pmCurrencyExchange.currencyExchange(balanceRacc, currencyIdOrigin, parityOrigin, currencyIdDestiny, parityDestiny);
		   ...
		   <%
		    ...
		    if (amountT > 0 || taxT > 0 || withDrawTotal > 0 || depositTotal > 0  || balanceT > 0 ) {
		    %>
	...
	// Si existe moneda destino
	    ...
	    sql = "   SELECT DISTINCT(racc_customerid) AS cliente,  " +
	    		//" cust_customerid, cust_code, cust_legalname, cust_displayname, orde_total, cureOrder.cure_currencyid, cureOrder.cure_code " +
	    		" cust_customerid,cust_code, cust_legalname, cust_displayname,cust_customercategory" +
	    		" FROM " + SQLUtil.formatKind(sFParams, " raccounts ") ;
	    if (enableBudgets) {
	    	sql += " LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitems") + " ON (bgit_budgetitemid = racc_budgetitemid) " 
	   ...
	   int xx=0;
	    while (pmCustomer.next()) {
	    	if ((pmCustomer.getInt("cust_customerid") != customerid || xx == 0)){
	    		 customerid = pmCustomer.getInt("cust_customerid");
	    		 bmoCustomer.getCustomercategory().setValue(pmCustomer.getString("customers","cust_customercategory"));
	    		   xx =2;
	    		   //Conversion del pedido a la moneda destino(seleccion del filtro) 
	   ...
	   <% 		} 
	       ...
	       while (pmRaccounts.next()) {
			   ...
			   <tr class="reportCellEven">
				   ...
				   <%	if (enableBudgets) { %>
	        		<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmRaccounts.getString("budgets", "budg_name")+" : "+ pmRaccounts.getString("budgetitemtypes", "bgty_name"), BmFieldType.STRING)) %>
		            <%= HtmlUtil.formatReportCell(sFParams, pmRaccounts.getString("areas","area_name"), BmFieldType.STRING) %>
		           <%	} %>
				   <%= HtmlUtil.formatReportCell(sFParams, pmRaccounts.getString("orders","orde_code"), BmFieldType.CODE) %>
				   <%	if (enableBudgets) { %>
				   <%= HtmlUtil.formatReportCell(sFParams, budget, BmFieldType.STRING) %>
