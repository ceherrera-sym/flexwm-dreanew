...
<%
	// Inicializar variables
 	String title = "Reporte se Seguimiento de Cobranza";
	...
	BmoCustomer bmoCustomer = new BmoCustomer();
   	...
   	String filters = "", customer = "", customerCategory ="", areaId = "-1", budgetItemId = "";
...
<table border="0" cellspacing="0" cellpading="0" width="100%">
	...
	<tr>
		<td class="reportSubTitle">
			...
			<%	}%>
			<b>Ordenado por:</b> <a><%   if(status.equals("A : ")){
		    	 Titulo = "Clave CxC";		    }else{
		    		 Titulo = "Factura";
		    } %></a><%=Titulo %>
		</td>
...
<table class="report" border="0">               
    ...
    String orden = "";
    if(status.equals("A : ")){
    	 orden = " ORDER BY racc_serie, racc_folio ASC";
    }else{
    	 orden = " ORDER BY racc_raccountid  ASC";
    }
		    	...
		    	%>       
			            ...
			            <tr class="">
		    			   <td class="reportHeaderCellCenter">#</td>	
							...
							%>
									<td class="reportHeaderCell">
									<%= HtmlUtil.stringToHtml(sFParams.getFieldFormTitle(bmoProgram.getCode().toString(), bmoRaccount.getStatus().getLabel()))%>
									</td>
							<%	}%>	
							...
							<%	}%>
							  <%	if (sFParams.isFieldEnabled(bmoRaccount.getStatusCharge())) {
									dynamicColspan++;
							%>
									<td class="reportHeaderCell">
									<%= HtmlUtil.stringToHtml(sFParams.getFieldFormTitle(bmoProgram.getCode().toString(), bmoRaccount.getStatusCharge()))%>
									</td>
							<%	}%>
...
// 		    			" AND ract_type = '" + BmoRaccountType.TYPE_WITHDRAW + "'" + 
		       ...
		       while (pmRaccounts.next()) {
		           ...
		           bmoCustomer.getCustomercategory().setValue(pmRaccounts.getString("customers","cust_customercategory"));
		   				...
		   				customer =  pmRaccounts.getString("cust_displayname");
		   %>
			...
			<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(bmoRaccount.getStatus().getSelectedOption().getLabel()), BmFieldType.STRING) %>
					   ...
					   <%	} %>
					     <%	if (sFParams.isFieldEnabled(bmoRaccount.getStatusCharge())) { %>	
					   <%= HtmlUtil.formatReportCell(sFParams, pmRaccounts.getString("raccounts","racc_statuscharge"), BmFieldType.STRING) %>
					   <%	} %>
					   					   <%	if (sFParams.isFieldEnabled(bmoRaccount.getCustomerId())) { %>
...
//					   balanceRacc = pmCurrencyExchange.currencyExchange(balanceRacc, currencyIdOrigin, parityOrigin, currencyIdDestiny, parityDestiny);
		   ...
		   %>
		               <tr class="reportCellEven reportCellCode">
		                    <td colspan="<%= (dynamicColspan-3)%>">&nbsp;</td>
		                    <%	if (sFParams.isFieldEnabled(bmoRaccount.getAmount())) { %>
		   ...
		   <%
		          ...
		          //sin moneda clave carlospueblasinmonerdalol.
		    if (withDrawTotal > 0 || depositTotal > 0) {
		    %>
		    	<tr><td colspan="<%= (dynamicColspan-4)%>">&nbsp;</td></tr>
			    <tr class="reportCellEven reportCellCode">
				    <%= HtmlUtil.formatReportCell(sFParams, "" + y, BmFieldType.NUMBER) %>
				    <td colspan="<%= (dynamicColspan-4)%>">&nbsp;</td>
				    <%	if (sFParams.isFieldEnabled(bmoRaccount.getAmount())) { %>
...
// 	   					+ " AND ract_type = '" + BmoRaccountType.TYPE_WITHDRAW + "'" 
	   ...
	   <%
	    ...
	    if (withDrawTotal > 0 || depositTotal > 0) {
	    %>
	    <tr><td colspan="<%= (dynamicColspan-3)%>">&nbsp;</td></tr>
		    <tr class="reportCellEven reportCellCode">
			    <%= HtmlUtil.formatReportCell(sFParams, "" + y, BmFieldType.NUMBER) %>
			    <td colspan="<%= (dynamicColspan-4)%>">&nbsp;</td>
			    <%	if (sFParams.isFieldEnabled(bmoRaccount.getAmount())) { %>
