...
<%
	// Inicializar variables
 	String title = "Reporte CxC a Detalle ";
	...
	BmoCustomer bmoCustomer = new BmoCustomer();
   	...
   	String sql = "", where = "", whereAreaId = "", sqlCurrency = "", receiveDate = "", dueDate = "", receiveEndDate = "", dueEndDate = "";
   	String detailReportLabel = " ", status = "", paymentStatus = "", paymentStatus2 = "", filters = "", customer = "", customerCategory ="", budgetItemId = "", areaId = "";
   	int programId = 0, customerid = 0, orderId = 0, cols= 0, raccountTypeId = 0, companyId = 0, wflowCategoryId = 0, userId = 0, collectorUserId = 0, 
   			currencyId = 0, failure = 0, linked = 0, budgetId = 0, activeBudgets = 0, detailReport = 2;
...
//    	if (request.getParameter("detailReport") != null) detailReport = Integer.parseInt(request.getParameter("detailReport"));
//    	if (request.getParameter("detailReportLabel") != null) detailReportLabel += request.getParameter("detailReportLabel");
	...
	if (enableBudgets) {
	    ...
	    if (!areaId.equals("")) {
	        where += SFServerUtil.parseFiltersToSql("rait_areaid", areaId);
	        //where += " AND rait_areaid = " + areaId;
...
<html>
	...
	if (exportExcel.equals("1") && sFParams.hasPrint(bmoProgram.getCode().toString())) {
		...
		response.setHeader("Content-Disposition", "inline; filename=\""+title + detailReportLabel +".xls\"");
	}
...
<head>
	<title>:::<%= title + detailReportLabel %>:::</title>
...
// 		while (pmCurrencyWhile.next()) {} // Fin pmCurrencyWhile
// 	}
// 	// Si existe moneda destino
// 	else {
		
		// Clientes que existen en CxC
// 		sql = " SELECT DISTINCT(racc_customerid), cust_code, cust_displayname, cust_legalname, cust_customertype FROM " + SQLUtil.formatKind(sFParams, "raccountitems");
// 		if (enableBudgets) {
// 				sql += " LEFT JOIN " + SQLUtil.formatKind(sFParams, "budgetitems") + " ON (bgit_budgetitemid = rait_budgetitemid) " 
// 						+ " LEFT JOIN "	+ SQLUtil.formatKind(sFParams, "budgetitemtypes") + " ON (bgit_budgetitemtypeid = bgty_budgetitemtypeid) " 
// 						+ " LEFT JOIN "	+ SQLUtil.formatKind(sFParams, "budgets") + " ON (budg_budgetid = bgit_budgetid) "
...
// 				+ whereAreaId 
// 				+ where
// 				+ " ORDER BY racc_customerid ASC ";
				
// 				System.out.println("sql_cl:"+sql);
// 				pmCustomers.doFetch(sql);
		int byCustomerId = 0;
		boolean headerResumeReport = true;
...
// 			if ((pmCustomers.getInt("racc_customerid") != byCustomerId)) {
// 				byCustomerId = pmCustomers.getInt("racc_customerid");
				// reiniciar sumatorias por cliente
				dynamicColspan = 0; dynamicColspanMinus = 0;
				if (detailReport == 2) x = 1; // reiniciar si es detalle 
				amount = 0;
				tax = 0;
				total = 0;
				payments = 0;
...
<!-- 	                </tr> -->
					...
					<!-- Tipo de pedido -->
						...
						%>
								<td class="reportHeaderCell">
									<%= HtmlUtil.stringToHtml(sFParams.getFieldFormTitle(bmoProgram.getCode().toString(), bmoOrder.getOrderTypeId()))%>
								</td>
						<%	}%>
							<!-- Tipo de flujo -->
						<%	if (sFParams.isFieldEnabled(bmoRaccount.getOrderId())) {
						...
						%>
								<td class="reportHeaderCell">
									<%= HtmlUtil.stringToHtml(sFParams.getFieldFormTitle(bmoProgram.getCode().toString(), bmoOrder.getWFlowTypeId()))%>
								</td>
						<%	}%>
...
// 			}
			...
			sql +=" LEFT JOIN " + SQLUtil.formatKind(sFParams, "raccounts")	+ " ON (racc_raccountid = rait_raccountid) " 
					...
					+ " LEFT JOIN "	+ SQLUtil.formatKind(sFParams, "companies") + " ON (racc_companyid = comp_companyid) "
					+ " WHERE racc_raccountid > 0 "
// 					+ " AND racc_customerid = " + pmCustomers.getInt("racc_customerid")
					+ where
					+ " ORDER BY racc_raccountid ASC, rait_name ASC ";
			...
			while (pmRaccounts.next()) {
				...
				if (pmRaccounts.getString("cust_customertype").equals("" + BmoCustomer.TYPE_COMPANY))
		   		...
		   		<% 	// Si es detalle mostrar columnas
		   			if (detailReport == 2) { %>
					<tr class="reportCellEven">
			        	...
			        	<%	if (sFParams.isFieldEnabled(bmoRaccount.getStatus())) { %>
					   	...
					   	<%	} %>
							<!-- Tipo de pedido -->
					 		<%	if (sFParams.isFieldEnabled(bmoRaccount.getOrderId())) { %>
						   			<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmRaccounts.getString("ordertypes","ortp_name")), BmFieldType.STRING) %>                    		   
						   	<%	} %>
		 				  	<!-- Tipo de pedido -->
					 		<%	if (sFParams.isFieldEnabled(bmoRaccount.getOrderId())) { %>
						   			<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmRaccounts.getString("wflowtypes","wfty_name")), BmFieldType.STRING) %>                    		   
						   	<%	} %>
					   	<%	if (sFParams.isFieldEnabled(bmoRaccount.getCustomerId())) { %>
				   	...
				   	if (!(currencyId > 0)) {
				   		currencyIdDestiny = pmRaccounts.getInt("racc_currencyid");
				   		parityDestiny = pmRaccounts.getDouble("racc_currencyparity");
				   	} else {
					...
					double totalRait = amountRait + taxRait;
				   	...
				   	balance += balanceRait;					
				  //Si es de tipo Nota de Crídito no sumar las cantidades a los totales
				   	...
				   	<% 	if (detailReport == 2) { %>
					        ...
					        <%
					        // Aumentar en caso de que sea detalle, ya que es a nivel de linea-item
					        x++; y++;
						}
	       	} // Fin pmRaccounts
	  		...
	  		if (currencyId > 0  && amount > 0) {
						if (detailReport == 1) { %>
	      				    <tr class="">
	      				    <%= HtmlUtil.formatReportCell(sFParams, "" + x, BmFieldType.STRING) %>
	  		...
	  		<%
		  		// Si es resumen aumentar por cliente
		  		if (detailReport == 1) { x++; y++; }
	  		}
// 		} // FIN de pmCustomers


	    if (currencyId > 0 && amountTotal > 0) {
	    %>
	    	<tr><td colspan="<%= (dynamicColspan)%>">&nbsp;</td></tr>
		    <tr class="reportCellEven reportCellCode">
		    <%= HtmlUtil.formatReportCell(sFParams, "" + y, BmFieldType.STRING) %>
		    	<% 	if (detailReport == 1) { %>
		    			<td>&nbsp;</td>
		    	<%	} else { %>
