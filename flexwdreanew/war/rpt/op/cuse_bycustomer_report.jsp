...
<%
    ...
    String title = "Reportes Agrupado por Cliente de Atencion a Clientes";
    ...
    String sql = "", where = "", filters = "", groupFilter = "", customer = "", phone = "", 
    ...
    int cols= 0, programId = 0,  customerServiceTypeId = 0, orderId = 0, customerId = 0, developmentId = 0, userId = 0;
 ...
 // Filtros
    ...
    PmConn pmConnCustomerPhone = new PmConn(sFParams);
	pmConnCustomerPhone.open();
...
<html>
	...
	if (sFParams.hasPrint(bmoProgram.getCode().toString()) || !(exportExcel.equals("1"))) { %>
		...
		<table class="report" width="100%">
			...
			// Clientes de las ventas, para la agrupacion
			sql = " SELECT DISTINCT(orde_customerid) AS cliente, cust_customerid, cust_code, cust_displayname, cust_legalname, " +
					" cust_customertype, cust_phone "+
					" FROM " + SQLUtil.formatKind(sFParams, " customerservices ") +     
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customerservicetypes")+" ON(csty_customerservicetypeid = cuse_customerservicetypeid) " +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" ON(orde_orderid = cuse_orderid) " +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " users")+" ON(user_userid = cuse_userid) " +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customers")+" ON(cust_customerid = orde_customerid) " +
			...
			pmConnCustomer.doFetch(sql);
			
			int y = 1, n = 0;
			while (pmConnCustomer.next()) {
				if (pmConnCustomer.getString("cust_customertype").equals(""+BmoCustomer.TYPE_COMPANY))
					customer = pmConnCustomer.getString("cust_code") + " " + pmConnCustomer.getString("cust_legalname");	
				else
					customer = pmConnCustomer.getString("cust_code") + " " + pmConnCustomer.getString("cust_displayname");

				//Telefonos del Cliente
				if (pmConnCustomer.getString("customers", "cust_phone").equals("")) {
					String sqlCustPhones = " SELECT * FROM " + SQLUtil.formatKind(sFParams, " customerphones ") +
							" WHERE cuph_customerid = " + pmConnCustomer.getInt("customers", "cust_customerid") + 
							" ORDER BY cuph_customerphoneid";
					pmConnCustomerPhone.doFetch(sqlCustPhones);
					//System.out.println("sqlPh: "+sqlCustPhones);
					if(pmConnCustomerPhone.next()) phone = pmConnCustomerPhone.getString("customerphones", "cuph_number");

				} else {
					phone = pmConnCustomer.getString("customers", "cust_phone");
				}
              
              %>
		    	<tr  class="reportGroupCell" >
			    	<td colspan="15" class="reportGroupCell">
			    		#<%= y%>&nbsp|&nbsp;<%= HtmlUtil.stringToHtml(customer) %> | Tel. <%= phone %>
			    	</td>
		    	</tr>
		    	<tr class="">
					<th class="reportHeaderCellCenter" width="1%">#</td>
			        <th class="reportHeaderCell">Clave</td>
			        ...
			        <th class="reportHeaderCell">Tipo Atn.<br>Cliente</td>
			    	<!--<th class="reportHeaderCell">Fecha<br>Venta</td>-->
			    	<th class="reportHeaderCell">Fecha<br>Entrega</td>
			    	<th class="reportHeaderCellCenter">D&iacute;as<br>Soluci&oacute;n</td>
			        <th class="reportHeaderCellCenter">D&iacute;as<br>en Espera</td>
				...
				sql = " SELECT * FROM " + SQLUtil.formatKind(sFParams, " customerservices ") +     
					    ...
					    " LEFT JOIN " + SQLUtil.formatKind(sFParams, " developments")+" ON(deve_developmentid = dvph_developmentid) " +
					    " WHERE cuse_customerserviceid > 0 " + 
					    " AND cust_customerid = " + pmConnCustomer.getInt("cust_customerid") +
					    where +
				                 ...
				                 <%
				                 	...
				                 	while(pmCustomerServiceFollowups.next()){
				                 		customerServiceFollowups += "<li>"+
				                 									...
				                 									"Fecha: " + pmCustomerServiceFollowups.getString("csfo_followupdate") + "&nbsp;|&nbsp;" +
			                 										"Comentario: " + pmCustomerServiceFollowups.getString("csfo_description")+
		             										  "</li>";
				                 	}
				                 %>
