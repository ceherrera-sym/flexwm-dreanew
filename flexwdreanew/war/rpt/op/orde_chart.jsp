...
<%
	...
	if (sFParams.getSelectedCompanyId() > 0) {
		where += " AND orde_companyid = " + sFParams.getSelectedCompanyId();
	}
   	
   	sql = " SELECT wfty_name AS Tipo, count(orde_orderid) AS Pedidos "
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          	...
          	<% 
       		...
       		%>
	        		['<%= pmConn.getString("Tipo") %>', <%= pmConn.getInt("Pedidos") %>],
        	<%
       			}
        	%>
        	]);
        ...
        var options = {
          chart: {
            ...
            subtitle: 'Por Tipo de Pedido, total: <%= total %>',
          },
