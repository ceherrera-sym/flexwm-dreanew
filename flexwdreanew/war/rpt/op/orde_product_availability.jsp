...
<table class="report">
    <tr class="">
        <td class="reportHeaderCell">Existencia Almacen</td>
        <td class="reportHeaderCell">O.C. Compra Aut. Ligadas</td>
		<td class="reportHeaderCell">O.C. Renta Aut. Ligadas</td>
		<td class="reportHeaderCell">Total</td>
        <td width="2">&nbsp;</td>
        <td class="reportHeaderCell">Bloqueo en Proyectos</td>
        <td width="2">&nbsp;</td>
        <td class="reportHeaderCell">Disponibilidad</td>
    </tr>
	<tr class="reportCellEven">
		...
		<td class="reportCellEven">
			<%
				String warehouseSql = "SELECT ware_code, ware_name, whse_name, SUM(whst_quantity) as q FROM " + SQLUtil.formatKind(sFParams, " whstocks ") +
				...
				pmConn.doFetch(warehouseSql);
				int i = 1;
				double warehouseTotal = 0;
				while (pmConn.next()) {
					warehouseTotal += pmConn.getDouble("q");
			%>
							<b><%= pmConn.getString("q") %></b>: 
							<%= pmConn.getString("ware_code") %>
							<br>
			<% } %>
		</td>
		...
		<!-- ORDENES COMPRA TIPO COMPRA -->
		<td class="reportCellEven">
				<%
					String requisitionPurchaseSql = "SELECT reqi_code, reqi_name, rqit_quantity, orde_code FROM " + SQLUtil.formatKind(sFParams, " requisitionitems ") 
				...
				%>
							<b><%= pmConn.getString("rqit_quantity") %></b>:
							<%= pmConn.getString("reqi_code") %>
		...
		<!-- ORDENES COMPRA TIPO RENTA -->
		<td class="reportCellEven">
					<%
						String requisitionRentalSql = "SELECT reqi_code, reqi_name, rqit_quantity, orde_code FROM " + SQLUtil.formatKind(sFParams, " requisitionitems ") 
									+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, " requisitions")+" ON (rqit_requisitionid = reqi_requisitionid) "
									+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, " requisitiontypes")+" ON (reqi_requisitiontypeid = rqty_requisitiontypeid) "
									+ " LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" ON (reqi_orderid = orde_orderid) "
									+ " WHERE rqit_productid = " + bmoProduct.getId()
		...
		<!-- BLOQUEO EN PROYECTOS -->
		<td class="reportCellEven">
				<table class="report">
			    	<tr>
			    			<td class="reportHeaderCell">Cant.</td>
			    			<td class="reportHeaderCell">Cant. Bloq.</td>
			    			<td class="reportHeaderCell">Cant. Confl.</td>
		  		...
		  		<%
			          ...
			          i = 1;
			          double total = 0;
			          double totalQuantity = 0, totalLocked = 0, totalConflict = 0;
			          while(pmConn.next()) {  
			          	totalQuantity += pmConn.getDouble("ordi_quantity");			          
			          	totalLocked += pmConn.getDouble("ordi_lockedquantity");
			          	totalConflict += pmConn.getDouble("ordi_conflictquantity");			          	
			    %>
			    	<tr>
			    			<td class="reportCellEven" align="center"><%= pmConn.getString("ordi_quantity") %></td>
			    			<td class="reportCellEven" align="center"><%= pmConn.getString("ordi_lockedquantity") %></td>
			    			<td class="reportCellEven" align="center"><%= pmConn.getString("ordi_conflictquantity") %></td>
			    			<td class="reportCellEven" align="center"><%= pmConn.getString("orde_code") %></td>
					</tr>
			    <%
			       i++;
			       }
			     %>
			    	<tr class="reportHeaderCellCenter">
			    			<td class="reportCellEven"><b><%= totalQuantity %></b></td>
			    			<td class="reportCellEven"><b><%= totalLocked %></b></td>
			    			<td class="reportCellEven"><b><%= totalConflict %></b></td>
	...
	<tr class="reportCellEven">
		<td class="reportHeaderCellCenter" align="center">
			+<%= warehouseTotal %>
		</td>
		<td class="reportHeaderCellCenter" align="center">
			+<%= requisitionPurchaseQuantity %>
		</td>
		<td class="reportHeaderCellCenter" align="center">
			+<%= requisitionRentalQuantity %>
		</td>
		<td class="reportHeaderCellCenter" align="center">
			+<%= warehouseTotal + requisitionPurchaseQuantity + requisitionRentalQuantity %>
		</td>
        <td>&nbsp;</td>
		<td class="reportHeaderCellCenter" align="center">
			<%= totalQuantity %>
        ...
        <td>&nbsp;</td>
		<td class="reportHeaderCellCenter" align="center">
			<%= warehouseTotal + requisitionPurchaseQuantity + requisitionRentalQuantity - totalQuantity %>
		</td>
	</tr>
