...
<%
	// Inicializar variables
 	String title = "Asignacion de Personal en Pedidos";
	...
	where += " AND ( "
   	...
   	sql = " select * FROM " + SQLUtil.formatKind(sFParams, " orderstaff ") +
   			" LEFT JOIN " + SQLUtil.formatKind(sFParams, " programprofiles")+" on (ords_usergroupid = pfus_usergroupid) " +
			" LEFT JOIN " + SQLUtil.formatKind(sFParams, " users")+" on (pfus_userid = user_userid) " +
			" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" on (ords_orderid = orde_orderid) " +
			" where ords_orderstaffid > 0 " + 
			where;
		...
		function drawChart() {
		  ...
		  <% while (pmConn.next()) { %>
		  		<%
					...
					equipmentStartYear = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockstart")).get(Calendar.YEAR);
					equipmentStartMonth = "" + (SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockstart")).get(Calendar.MONTH) + 1);
					equipmentStartDay = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockstart")).get(Calendar.DAY_OF_MONTH);		 	
					equipmentStartHour = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockstart")).get(Calendar.HOUR_OF_DAY);		 	
					equipmentStartMinute = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockstart")).get(Calendar.MINUTE);		 	
					
					equipmentEndYear = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockend")).get(Calendar.YEAR);
					equipmentEndMonth = "" + (SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockend")).get(Calendar.MONTH) + 1);
					equipmentEndDay = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockend")).get(Calendar.DAY_OF_MONTH);
					equipmentEndHour = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockend")).get(Calendar.HOUR_OF_DAY);		 	
					equipmentEndMinute = "" + SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("ords_lockend")).get(Calendar.MINUTE);	
		  				  		
		  		%>
		  
		  
		  	[ '<%= HtmlUtil.stringToHtml(pmConn.getString("user_firstname")) %> <%= HtmlUtil.stringToHtml(pmConn.getString("user_fatherlastname")) %>', '<%= pmConn.getString("orde_code") %>', new Date(<%= equipmentStartYear %>, <%= equipmentStartMonth %>, <%= equipmentStartDay %>, <%= equipmentStartHour %>, <%= equipmentStartMinute %>, 00), new Date(<%= equipmentEndYear %>, <%= equipmentEndMonth %>, <%= equipmentEndDay %>, <%= equipmentEndHour %>, <%= equipmentEndMinute %>, 00) ],
...
<table class="report">
	<tr>
		<td colspan="12" class="reportGroupCell">
			Asignacion de Personal en el Intervalo de Fechas
		</td>
	</tr>

    <tr class="">
        <td class="reportHeaderCellCenter">#</td>
        <td class="reportHeaderCell">Persona</td>
    ...
    <%
	    ...
	    %>
	          <tr class="reportCellEven">
						...
						<%= HtmlUtil.formatReportCell(sFParams, pmConn.getString("ords_lockstart"), BmFieldType.STRING) %>
						
						<%= HtmlUtil.formatReportCell(sFParams, pmConn.getString("ords_lockend"), BmFieldType.STRING) %>
	          </tr>
	    <%
	       i++;
	       }
...
<table class="report">
	<tr>
		<td colspan="12" class="reportGroupCell">
			Personal Disponible en el Intervalo de Fecha y Hora exclusivos del Pedido "<%= bmoOrder.getCode().toString() %> <%= HtmlUtil.stringToHtml(bmoOrder.getName().toString()) %>"
		</td>
	</tr>
    <tr class="">
	...
	<%
		String availableSql = "SELECT * FROM " + SQLUtil.formatKind(sFParams, " users ") 
							...
							+ " WHERE user_userid NOT IN ( "
								+ " select pfus_userid FROM " + SQLUtil.formatKind(sFParams, " orderstaff ") +
   								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " programprofiles")+" on (ords_usergroupid = pfus_usergroupid) " +
								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " users")+" on (pfus_userid = user_userid) " +
								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" on (ords_orderid = orde_orderid) " +
								" where ords_orderstaffid > 0 " + 
								 " AND ( "
								   	+ " ('" + bmoOrder.getLockStart().toString() + "' BETWEEN ords_lockstart AND ords_lockend )" 
								   	+ " OR "
									+ " ('" + bmoOrder.getLockEnd().toString() + "' BETWEEN ords_lockstart AND ords_lockend )" 
								   	+ " ) " 
							+ ")";
	...
	%>
		<tr class="reportCellEven">
			...
			<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConn.getString("user_email"), BmFieldType.STRING)) %>
			
			<%= HtmlUtil.stringToHtml(HtmlUtil.formatReportCell(sFParams, pmConn.getString("area_name"), BmFieldType.STRING)) %>	
		</tr>
	<% } %>
</table>   
<% } %>
...
<table class="report">
	<tr>
		<td colspan="12" class="reportGroupCell">
			Personal Disponible en el Intervalo de Fecha y Hora Ampliado
		</td>
	</tr>
    <tr class="">
        <td class="reportHeaderCell">#</td>
	...
	<%
		String availableSql = "SELECT * FROM " + SQLUtil.formatKind(sFParams, " users ") 
							...
							+ " WHERE user_userid NOT IN ( "
								...
								" where ords_orderstaffid > 0 " + 
								" AND ( "
								+ " (ords_lockstart BETWEEN '" + startString + "' AND '" + endString + "' )" 
							 	+ " OR "
								+ " (ords_lockend BETWEEN '" + startString + "' AND '" + endString + "' )" 
							 	+ " ) "
							+ ")";
		pmConn.doFetch(availableSql);
		i = 1;
