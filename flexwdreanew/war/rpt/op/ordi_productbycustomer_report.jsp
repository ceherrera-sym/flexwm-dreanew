...
<%
    ...
    String title = "Reportes de Productos en Pedidos Agrupado por Cliente";
	...
	String filters = "", sql = "", sqlGroup = "", where = "", whereOrder = "", whereProduct = "", whereExtra = "", lockStartDate = "", lockEndDate = "", 
   	...
   	int programId = 0, rows = 0, orderId = 0, ordeId = 0, industryId = 0, customerId = 0, productId = 0, areaId = 0, userId = 0, 
   			showProductExtra = 0, currencyId = 0,wflowtypeId=0, dynamicColspan = 0, dynamicColspanMinus = 0;
...
<table bprodr="0" cellspacing="0" cellpading="0" width="100%">
    ...
    <tr>
        <td class="reportSubTitle">
            <b>Filtros:</b> <%= filters %>
            <br>
            <b>Agrupado por:</b> <%= ((!(currencyId > 0)) ? "Moneda, Cliente, Producto." : "Cliente, Producto.")%>
...
<table class="report">
	...
	if (!(currencyId > 0)) {
		...
		while (pmCurrencyWhile.next()) {
			...
			sqlGroup = " SELECT cust_customerid, cust_code, cust_displayname, cust_legalname, cust_status, cust_customertype FROM " + SQLUtil.formatKind(sFParams, " customers") +
		    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" ON(cust_customerid = orde_customerid) " +
		    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, " users")+" ON(user_userid = orde_userid) " +
		    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, " areas")+" ON(area_areaid = user_areaid) " +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " currencies")+" ON (orde_currencyid = cure_currencyid) " +
		    ...
		    while (pmCustomer.next()) {
				...
				if (countItem > 0) {
					if (pmCustomer.getString("cust_customertype").equals(""+BmoCustomer.TYPE_COMPANY))
	      		  		fullName = pmCustomer.getString("cust_code") + " " + pmCustomer.getString("cust_legalname");	
	      		  	else
	      		  		fullName = pmCustomer.getString("cust_code") + " " + pmCustomer.getString("cust_displayname");
					
					//Estatus
					bmoCustomer.getStatus().setValue(pmCustomer.getString("customers", "cust_status"));
					%>
					<tr class="">
						<td class="reportGroupCell" colspan="9">
						<b>
							# <%= countCust %> |
							<%= HtmlUtil.stringToHtml(fullName)%>
							| <%= HtmlUtil.stringToHtml(bmoCustomer.getStatus().getSelectedOption().getLabel())%>
						</b>
						</td>
					</tr>
					<tr class="">
