...
<%
	// Inicializar variables
 	String title = "Uso de Recursos en Pedidos";
   	...
   	String sql = "", where = "", filters = "", ordeStatus = "", paymentStatus = "", deliveryStatus = "", lockStatus  = "", fullName = "", sqlCurrency = "";
   	int programId = 0,wflowtypeId=0, orderId = 0, rows = 0, customerId = 0, equipmentId = 0, areaId = 0, userId = 0, industryId = 0, 
	...
	int endYear = 0, endMonth = 0, endDay = 0, endHour = 0, endMinute = 0;	
   	...
   	PmEquipment pmEquipment = new PmEquipment(sFParams);
    ...
    if (request.getParameter("orde_orderid") != null) orderId = Integer.parseInt(request.getParameter("orde_orderid"));    
   	...
   	if (request.getParameter("equi_equipmentid") != null) equipmentId = Integer.parseInt(request.getParameter("equi_equipmentid"));
    ...
    if (equipmentId > 0) {
        where += " AND equi_equipmentid = " + equipmentId;
        filters += "<i>Recurso: </i>" + request.getParameter("equi_equipmentidLabel") + ", ";
    }
    ...
    if (currencyId > 0) {
  		...
  		sqlCurrency = " SELECT cure_currencyid, cure_code, cure_name, cure_parity FROM " + SQLUtil.formatKind(sFParams, " orderequipments ") +
...
if (!(currencyId > 0)) {
	...
	while (pmCurrencyWhile.next()) {
		...
		sql = " SELECT equi_name, equi_code, ordq_description, eqty_name, orde_code, orde_name, orde_lockstart, orde_lockend, " + 
				" orde_status, cust_code, cust_displayname, cust_customertype, cust_legalname, cure_code, indu_name FROM " + SQLUtil.formatKind(sFParams, " orderequipments ") +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " equipments")+" ON (ordq_equipmentid = equi_equipmentid) " +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " equipmenttypes")+" ON (eqty_equipmenttypeid = equi_equipmenttypeid) " +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" ON (ordq_orderid = orde_orderid) " +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customers")+" ON (orde_customerid = cust_customerid) " +
			...
			function drawChart() {
				...
				dataTable.addRows([
				  ...
				  */
		           <% 
		           while (pmConn.next()) {
			           int equipmentStartYear = 0, equipmentStartMonth = 0, equipmentStartDay = 0, equipmentStartHour = 0, equipmentStartMinute = 0;	
			           int equipmentEndYear = 0, equipmentEndMonth = 0, equipmentEndDay = 0, equipmentEndHour = 0, equipmentEndMinute;	
			           ...
			           equipmentStartMonth = (SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("orde_lockstart")).get(Calendar.MONTH));
			           ...
			           if (!pmConn.getString("orde_lockend").equals("")) {
			        	   ...
			        	   equipmentEndMonth = (SFServerUtil.stringToCalendar(sFParams.getDateTimeFormat(), pmConn.getString("orde_lockend")).get(Calendar.MONTH));
			        	   ...
			        	   %>
			        	   [ '<%= HtmlUtil.stringToHtml(pmConn.getString("equi_name")) %>', ' para Pedido: <%= pmConn.getString("orde_code") %> ', 
			        	     new Date(<%= equipmentStartYear %>, <%= equipmentStartMonth %>, <%= equipmentStartDay %>, <%= equipmentStartHour %>, <%= equipmentStartMinute %>, 00), 
			        	     new Date(<%= equipmentEndYear %>, <%= equipmentEndMonth %>, <%= equipmentEndDay %>, <%= equipmentEndHour %>, <%= equipmentEndMinute %>, 00) ],
			        	   <%
			        	   rows++;
			           }
		           }
		...
		<table class="report">
		    ...
		    while(pmConn.next()) {  
		    	...
		    	<tr class="reportCellEven">
			    	...
			    	<%= HtmlUtil.formatReportCell(sFParams, pmConn.getString("equi_code"), BmFieldType.CODE) %>
			
			    	<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmConn.getString("equi_name")), BmFieldType.STRING) %>
			    	
			    	<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmConn.getString("ordq_description")), BmFieldType.STRING) %>
			
			    	<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmConn.getString("eqty_name")), BmFieldType.STRING) %>
