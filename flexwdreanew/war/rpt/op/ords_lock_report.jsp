...
<%
	// Inicializar variables
 	String title = "Uso de Staff / Grupos en Pedidos";
   	String sql = "", where = "", filters = "", ordeStatus = "", paymentStatus = "", deliveryStatus = "", lockStatus  = "", sqlCurrency = "", fullName = "";
   	int programId = 0, orderId = 0, industryId = 0, areaId = 0, rows = 0, customerId = 0, userId = 0, currencyId = 0, dynamicColspan = 0, dynamicColspanMinus = 0;
    ...
    if (userId > 0) {
        ...
        /*where += " AND ( " +
					...
					" OR orde_wflowid IN ( " +
					 ...
					 " ) " + 
				 " )";*/
        filters += "<i>Personal: </i>" + request.getParameter("orde_useridLabel") + ", ";
    }
    ...
    if (currencyId > 0) {
  		...
  		sqlCurrency = " SELECT cure_currencyid, cure_code, cure_name, cure_parity FROM " + SQLUtil.formatKind(sFParams, " orderstaff ") +
  				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " profiles")+" ON (ords_profileid = prof_profileid) " +
...
if (!(currencyId > 0)) {
	...
	while (pmCurrencyWhile.next()) {
		...
		sql = " SELECT ords_name, orde_code, orde_name, orde_lockstart, orde_lockend, ords_description, " + 
				" orde_status, cust_code, cust_displayname, cust_customertype, cust_legalname, prof_name, cure_code, indu_name FROM " + SQLUtil.formatKind(sFParams, " orderstaff ") +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " profiles")+" ON (ords_profileid = prof_profileid) " +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" ON (ords_orderid = orde_orderid) " +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customers")+" ON (orde_customerid = cust_customerid) " +
				" LEFT JOIN " + SQLUtil.formatKind(sFParams, " industries")+" ON (indu_industryid = cust_industryid) " +
			...
			function drawChart() {
			  ...
			  dataTable.addRows([
			         ...
			         <%	while (pmConn.next()) { %>
			                 ...
			                 int staffEndYear = 0, staffEndMonth = 0, staffEndDay = 0, staffEndHour = 0, staffEndMinute;	
			                 ...
			                 if (!pmConn.getString("orde_lockend").equals("")) {
			                	 ...
			                	 [ '<%= HtmlUtil.stringToHtml(pmConn.getString("ords_name")) %>', ' para Pedido: <%= pmConn.getString("orde_code") %> ', 
		...
		<table class="report">
		    ...
		    while (pmConn.next()) {
		    	...
		    	<tr class="reportCellEven">
			    	...
			    	<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmConn.getString("prof_name")), BmFieldType.CODE) %>
			    	
			    	<%= HtmlUtil.formatReportCell(sFParams, HtmlUtil.stringToHtml(pmConn.getString("ords_name")), BmFieldType.STRING) %>
	...
	sql = " SELECT ords_name, orde_code, orde_name, orde_lockstart, orde_lockend, " + 
			" orde_status, cust_code, cust_displayname, cust_customertype, cust_legalname, prof_name, cure_code, indu_name FROM " + SQLUtil.formatKind(sFParams, " orderstaff ") +
    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, " profiles")+" ON (ords_profileid = prof_profileid) " +
    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" ON (ords_orderid = orde_orderid) " +
    		" LEFT JOIN " + SQLUtil.formatKind(sFParams, " customers")+" ON (orde_customerid = cust_customerid) " +
