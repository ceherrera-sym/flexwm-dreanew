...
<%
    ...
    String title = "Reporte O.C. Agrupado por Departamento";
	...
	String sql = "", sqlCurrency = "",  where = "", filters = "",
			reqiStatus = "", lockstatus = "", deliveryStatus = "", paymentStatus = "",
			deliveryStartDate = "", deliveryEndDate = "", requestStartDate = "", requestEndDate = "", dueStartDate = "", dueEndDate = "",
			userName = "", requestStatus = "", groupFilter="", areaId = "";
    ...
    if (!areaId.equals("")) {
    	//groupFilter += " AND reqi_areaid = " + areaId;
    	groupFilter += SFServerUtil.parseFiltersToSql("reqi_areaid", areaId);        
...
<table class="report">
	...
	if (!(currencyId > 0)) {
		...
		while (pmCurrencyWhile.next()) {
			...
			sql = " SELECT area_areaid, area_name FROM " + SQLUtil.formatKind(sFParams, " areas ") +
					" WHERE area_areaid IN (SELECT reqi_areaid FROM " + SQLUtil.formatKind(sFParams, " requisitions ")+ 
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " areas")+" on (area_areaid = reqi_areaid)" +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " orders")+" on (orde_orderid = reqi_orderid)" +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " companies")+" on (comp_companyid = reqi_companyid) " +
					" LEFT JOIN " + SQLUtil.formatKind(sFParams, " users")+" on (user_userid = reqi_requestedby) " +
			...
			while (pmAreas.next()) {
		    	...
		    	sql += " WHERE reqi_areaid = " + pmAreas.getInt("area_areaid") + 
		   	  			" AND reqi_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
		    			where +
		    			" ORDER BY reqi_requestdate ASC, reqi_requisitionid ASC";
				...
				while (pmRequisition.next()) {
					...
					if ((pmAreas.getInt("areas", "area_areaid") != areaid)) {
						areaid = pmAreas.getInt("areas", "area_areaid");
						%>      
						<tr>
							<td class="reportGroupCell" colspan="<%= (24 + activeBudgets)%>">
								<%= HtmlUtil.stringToHtml(pmAreas.getString("area_name")) %>                        
							</td>
						</tr>
						<tr class="">
