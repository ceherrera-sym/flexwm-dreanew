...
<%
    ...
    String title = "Reporte O.C. Agrupado por Proveedor";
    ...
    String reqiStatus = "", lockstatus = "", deliveryStatus = "", paymentStatus = "",
    	   ...
    	   userName = "", requestStatus = "", groupFilter="", filters = "", areaId = "";
    int programId = 0, companyId = 0, userId = 0, supplierId = 0, cols= 0, type = 0, budgetId = 0, budgetItemId = 0, orderId = 0, currencyId = 0, activeBudgets = 0;
    ...
    if (supplierId > 0) {
    	groupFilter += " AND reqi_supplierid = " + supplierId;
...
sql += " WHERE reqi_supplierid > 0" + 
	...
	if (!(currencyId > 0)) {
		...
		while (pmCurrencyWhile.next()) {
			...
			sql = " SELECT supl_supplierid ,supl_code, supl_name FROM " + SQLUtil.formatKind(sFParams, " suppliers ") + 
					" WHERE supl_supplierid in (SELECT reqi_supplierid FROM " + SQLUtil.formatKind(sFParams, " requisitions ");
					if (enableBudgets) {
						sql += " LEFT JOIN " + SQLUtil.formatKind(sFParams, " budgetitems")+" on (reqi_budgetitemid = bgit_budgetitemid)" +
								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " budgetitemtypes")+" on (bgit_budgetitemtypeid = bgty_budgetitemtypeid) " +
								" LEFT JOIN " + SQLUtil.formatKind(sFParams, " budgets")+" on (bgit_budgetid = budg_budgetid) ";
			...
			while(pmSupplier.next()) {
				...
				sql += " WHERE reqi_supplierid = " + pmSupplier.getInt("supl_supplierid") + 
		   	  			" AND reqi_currencyid =  " + pmCurrencyWhile.getInt("cure_currencyid") +
						where +
						" ORDER BY reqi_requestdate ASC, reqi_requisitionid ASC"; 
				...
				while(pmRequisition.next()) {
					...
					if ((pmSupplier.getInt("suppliers", "supl_supplierid") != supplierid)) {
						...
						%>      
						<tr>
							<td colspan="<%= (24 + activeBudgets)%>" class="reportGroupCell">
